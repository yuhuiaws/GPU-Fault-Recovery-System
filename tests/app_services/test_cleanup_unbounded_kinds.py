...
``decision``, ``event`` or ``regional_registry_member``; every provider event
scanned the whole marker kind and every scrape aggregated the whole notification
kind. Each sweep here takes only settled rows past retention and keeps anything
an incident that still exists references -- the archiver bundles those with the
incident (F-I1).
"""
