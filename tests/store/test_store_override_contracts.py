...
# The three stores are peers composed from shared mixins (S13). The memory
# store is still the reference shape; the shared record templates run on both
# SQLite and PostgreSQL, so they sit with the SQLite layer's overrides and with
# the PostgreSQL layer's bases.
MEMORY_LAYER = (
    "InMemoryStore",
...
LAYERS = (
    (SQLITE_LAYER + SHARED_RECORD_LAYER, MEMORY_LAYER + SHARED_RULES_LAYER),
    (
        POSTGRES_LAYER,
        SHARED_RECORD_LAYER + SHARED_RULES_LAYER + SQLITE_LAYER + MEMORY_LAYER,
    ),
)
