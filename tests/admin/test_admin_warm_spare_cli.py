...
def test_the_plain_config_path_parses_without_a_reference(tmp_path: Path) -> None:
    # ``--reference`` is not on the parser's required list, so a read-only
    # ``config spare`` needs none; the audited apply defaults it to the approver
    # identity and the start time (``default_admin_config_reference``).
    parsed = cli.parser().parse_args(["config", "--state-dir", str(tmp_path)])
    assert parsed.reference is None, parsed
    assert parsed.config_action is None, parsed
