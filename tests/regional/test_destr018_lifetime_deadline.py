...
def test_the_case_chooses_two_variables_and_the_env_window_derives_the_rest() -> None:
    ...
    # The six the runner sets explicitly, plus the two install knobs the window
    # derives from the lifetime (an install ceiling above the lifetime is
    # refused at boot just like the step ceilings).
    assert set(env_window.ALLOWED_VARIABLES) == {
        "GPU_FAULT_NODE_WORKFLOW_MAX_LIFETIME_SECONDS",
        "GPU_FAULT_WORKFLOW_EXECUTION_TIMEOUT_SECONDS",
...
def test_the_runner_opens_the_window_with_a_consistent_full_set(tmp_path: Path) -> None:
    ...
    # The window derives the install knobs the runner leaves to it; the set the
    # Deployment gets is the whole allow-list, and still bootable.
    completed = env_window.complete_assignments(assignments)
    assert set(completed) == set(env_window.ALLOWED_VARIABLES)
    assert env_window.assignment_errors(completed) == []
