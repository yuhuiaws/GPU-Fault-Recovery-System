...
| 代码版本 | 0.10.0 |
| 事实核对日期 | 源码、生成清单和测试契约基线：2026-09-15；已启用部署快照：2026-09-20（§4.1.2） |
| 语言与框架 | Python 3.12、FastAPI、Pydantic |
| 适用环境 | Kubernetes、EKS、SageMaker HyperPod EKS；核心模型可扩展到 EC2、Slurm |
| 文档依据 | `src/gpu_fault`、`src/gpu_fault_release`、`deploy`、`config`、`pyproject.toml` |
...
| 要部署或接管运维 | [§4.1.2 已启用部署架构](#deployment-architecture-snapshot) → §4.1.1 交互拓扑 → §7.1 区域形态 → [部署和运维手册](部署和运维手册.md) |
...
`src/` 全库对 Slurm 的引用只有四个文件，且都不是执行路径——
`models.py:97` 的 `Environment.HYPERPOD_SLURM`（裸 `slurm` 值已删除，解析期拒绝）、
`hyperpod.py:102-118` 的 `from_slurm_auto_resume()`（归属判断，且未接到运行时）
与 `:398` 的环境判定、`orchestration/coordinator.py:1549` 的环境分支，
以及 `watcher.py:181` 一句"Kubernetes/Slurm 通用"的注释。
...
`Environment` 枚举里的 `hyperpod-slurm` 只是数据模型取值，
**不表示该编排受支持**；本文所有 Slurm 相关描述
...
**只在 `observe()` 内被调用**（`managed_recovery.py:281`、`:289` 是唯一调用点），
没有 workflow step 就都不会发生。后果是旧 agent 停在 `ACTIVE` 不被 fence、
新节点不经本方案校验直接进调度池；更具体地，若新实例上的 agent 沿用同一
`node_id` 注册，`fleet.py:535-556` 的 `same_instance_reboot` 因
...
（`deploy/hyperpod/deploy.sh:2332-2339`），
...
（全库仅以只读方式用到 instance-group / cluster-name / node-health-status
选择器：`hma.py`、`hyperpod_spares.py`、`node_installer_reconciler.py`、
`failure_domains.py`，`admin/` 下的纳管、热备与故障域模块，以及
`src/gpu_fault_release` 的预检；没有任何写入），恢复调度时也只删除自有
...
系统（状态库、SNS/SES 管理员通知、可选的 S3 与 AMP）。
...
- **区域形态（生产）**：`Regional Remote Adapter` 只把动作写成命令，由目标集群的
  ...
  `Kubernetes Adapter` 与 HyperPod mutation adapter **必须禁用**，启用则启动失败
  （`settings.py`：regional mode 下这两个开关任一为 true 即 `RuntimeError`）。
- **单集群一体化形态（历史）**：这两个 Adapter 直连本地 API，没有 Executor。
...
| 持有的凭证 | 每集群 token 的 SHA-256 摘要（轮换期另有 retiring 摘要）、Aurora 凭证、站点 SNS 主题的 `sns:Publish`（`channel: ses` 站点为 `ses:SendEmail`） | 自己集群的 token、本地 ServiceAccount、Node Action 签名密钥 |
...
**图 4-2 GPU 集群侧组件。** Collector/Agent 在节点上以 systemd 单元运行，
dcgm-exporter 由 DaemonSet 提供，集群级组件以 Deployment 运行；
node-agent 的节点动作入口只被本集群的 executor 调用。

```mermaid
flowchart LR
    subgraph NODE["每 GPU 节点：运行时组件"]
    ...
    subgraph GDEP["GPU 集群内 Deployment"]
        ...
        ADP["adot-dataplane 1"]
        HMA["可选清单：hma-watcher / hma-cloudwatch-consumer（发布不部署）"]
    end
...
flowchart LR
    ...
    WRK --> NOTIFY["站点 SNS 主题（默认，与 AMP 告警共用）/ SES 邮件（channel: ses）"]
    ...
    AMP -.-> GRF["Amazon Managed Grafana（deploy 导入仪表盘）"]
```
...
| dcgm-exporter | GPU 集群 DaemonSet；节点安装器用 `--dcgm-exporter existing` 复用，不另启动同功能 systemd 服务 | 无 | `:9400`，供同节点 metrics-collector 抓取 | 无 |
...
| cluster-action-executor | GPU 集群 Deployment 2 副本 | 控制面领取/回执、本集群 Kubernetes API、node-agent `:9099`、SageMaker、S3 | 无 | 该集群 token + 本地 ServiceAccount + IRSA role（GPU 集群 OIDC 信任）+ Node Action 签名密钥 |
| node-installer-reconciler | GPU 集群 Deployment 1 副本 | **只连本集群 Kubernetes API**，不连控制面 | 无 | 本地 ServiceAccount |
| adot-dataplane | GPU 集群 Deployment 1 副本 | 抓本集群数据面 Pod `/metrics`（如 completion-watcher `:9109`），remote_write 到 AMP（与控制面同一工作区） | 无 | IRSA role（仅 `aps:RemoteWrite`） |
| hma-watcher / hma-cloudwatch-consumer | GPU 集群 Deployment，可选清单（`deploy/dataplane/optional/`，发布引擎不部署） | Kubernetes API 或 SQS + 控制面 | 无 | 该集群 token |
...
| control-worker | 控制面 Deployment 6 副本 × 4 进程 | Aurora、SNS 或 SES、SageMaker 只读 | `:8081`，不在 NLB target 中，只被探针与 ADOT 访问 | 同上 + `sns:Publish`（`channel: ses` 时为 `ses:SendEmail`） |
| spool-worker | 控制面 Deployment 0 副本（按需扩） | Aurora | `:8082`，同上 | 同上 |
| ADOT Collector（控制面） | 控制面 EKS | 抓三类角色 Pod IP `/metrics`，remote_write 到 AMP | 无 | execution token（Bearer） |
| aurora-credential-refresh | 控制面 CronJob | Secrets Manager | 无 | Pod Identity role |
| Amazon Managed Grafana | 区域托管服务，deploy 创建或复用 | 读 AMP（工作区 IAM role，SigV4） | 无 | IAM Identity Center 用户（管理员自动授 ADMIN） |
...
2. **node-agent 是节点动作的入向执行服务**，且只在 GPU VPC 内被本集群的
   cluster-action-executor 调用；其他组件的 metrics/health 监听不是节点动作入口。
   控制面不直连 `:9099`，只把动作写成
...
5. **AMP 告警与控制面通知共用一个 SNS 主题，但是两条独立路径**：
   ADOT→AMP→Alertmanager→SNS 覆盖控制面三类角色与各 GPU 集群数据面 Pod 的自身指标；
   collector 静默、workflow 与故障通知走控制面 Aurora outbox，再按
   `spec.notifications.channel` 发到同一 SNS 主题（默认）或 SES（`channel: ses`，
   仅老站点或显式选择；见 §4.2.1）。无 AMP 的 region 关掉前者，后者仍然生效。

### 4.1.2 已启用部署架构（2026-09-20 核对快照）

<a id="deployment-architecture-snapshot"></a>

下图记录 **2026-09-20 UTC、us-west-2** 的已启用部署：一个独立 CPU HyperPod/EKS
控制面和一个受管 GPU HyperPod/EKS 集群。它由 Kubernetes 清单、非敏感配置、Fleet、
安装资源注册表和 AWS 元数据的只读核对形成，节点内部职责再与当前实现对照。
**节点数、副本数、ACU 和 Ready 数是该时点的观察值，不是所有站点必须使用的固定配置，
也不代表已通过故障切换、压测或全部上线门禁。**

![图 4-4 已启用部署架构与 AWS 服务（2026-09-20 核对快照）](diagrams/deployment-architecture-20260920.svg)

本快照的控制面为 3 台 `ml.c5.4xlarge`，运行 ingress 3 副本、control-worker 6 副本和
ADOT 1 副本，并复用平台已有的 Load Balancer Controller。GPU 集群为 4 台
`ml.p5en.48xlarge`，运行 Executor 2 副本，Watcher、Node Resource Collector、
Installer Reconciler 和数据面 ADOT 各 1 副本；DCGM Exporter、NVIDIA/EFA 插件各 4/4，
四种节点 Collector 为 active/enabled。GPU HyperPod 保持 `NodeRecovery=None`。
受管训练 Pod 按业务需要创建，本次查询运行数为 0；安装和 schema Job 按需执行，
不按常驻组件计数。
...
| 服务 | 本次确认的用途与边界 |
|---|---|
| Amazon EKS / SageMaker HyperPod | 分别承载 CPU 控制面与 GPU 数据面；GPU 集群只由本地 Executor 执行获准操作 |
| Amazon EC2 / EBS | HyperPod 工作节点与数据卷；不替代区域共享状态库 |
| Amazon VPC | 两侧独立 VPC、子网、SG、路由、IGW 和 NAT/EIP；两侧均有 S3 Gateway Endpoint，CPU 侧另有 AMP/Grafana Interface Endpoint |
| ELB Network Load Balancer / Route 53 / ACM | 双 AZ 公网 NLB：TLS 443 终止后转发 ingress TCP 8080；私有 Hosted Zone 关联两侧 VPC并指向 NLB；使用导入的 ACM 证书，不将其误称为 AWS Private CA |
| Aurora PostgreSQL / RDS | Serverless v2 PostgreSQL 16.8，跨 AZ 的 writer/reader 各 1 个，每实例 8–32 ACU；保存队列、workflow、租约、回执与通知 outbox |
| Secrets Manager / KMS | RDS 托管数据库口令、区域 PKI 与托管加密；凭据刷新 CronJob 已部署，不向 GPU 下发 execution token |
| IAM / STS | CPU 组件使用 EKS Pod Identity，GPU Executor/ADOT 使用各自 IRSA；按组件授权临时 AWS 凭据 |
| Amazon Managed Service for Prometheus / Managed Grafana / IAM Identity Center | 两侧 ADOT 写入站点 AMP；Grafana 查询 AMP，通过 Identity Center 登录 |
| Amazon CloudWatch Logs | 两侧 EKS 的控制面/审计及平台日志，与业务 AMP 指标路径区分 |
| Amazon SNS | 当前站点通知通道；接收控制面 outbox 通知与 AMP 告警，本次有 1 个已确认订阅 |
| Amazon S3 / ECR | S3 控制记录归档已配置，HyperPod 生命周期脚本及现有业务 CSI 挂载使用 S3；ECR 分为 Runtime 镜像与 BuildKit cache 仓库 |

未画入常驻主图的项目包括：spool-worker（0 副本）、NodeLogCollector（disabled）、
training-progress health monitor（关闭）、MPS DaemonSet（0），以及发布未启用的
HMA watcher / SQS / Lambda / EventBridge 采集链、SES 通知和验收注入工具。
FSx CSI 驱动存在不等于本方案使用 FSx；本次没有把它列为故障系统的存储依赖。
Node Agent 的诊断包 S3 上传未确认启用，因此图中没有该上传箭头。
warm-spare Adapter 已启用也不等于当前有可用备用容量。

部署关系的代码依据为 `src/gpu_fault_release/regional_deployment_inventory.py`、
`src/gpu_fault_release/regional_release_rendering.py`、
`src/gpu_fault/admin/resource_registry.py` 和 `deploy/dataplane/`、`deploy/systemd/`。
SVG 已作为本地文档图源纳入仓库，不依赖私有 S3 链接；图中省略账号、ARN、私有地址和
具体集群名称。后续更新须重新核对部署并同步图源与核对日期，不能用本快照替代当前
`site`、配置模型或安装资源注册表。
...
| `KubernetesHmaNodeCollector` | Kubernetes HMA Node 对象 | 可选清单 `deploy/dataplane/optional/hma-watcher.yaml`，发布引擎不部署 |
| `CloudWatchHmaCollector` | CloudWatch HMA 订阅事件 | 可选清单 `optional/hma-cloudwatch-consumer.yaml`（SQS bridge），发布引擎不部署 |
...
`KubernetesNodeResourceCollector`。因此控制面状态、指标和通知必须同时显示两者：
...
AMP/ADOT 是可选增强，不是 collector 静默告警的必需组件。控制面默认以 60 秒间隔
检查已持久化的采集状态，范围限于活跃集群中 **ACTIVE 且 heartbeat lease 仍有效**
的 Agent 所需采集通道，显式 disabled/masked 的采集服务不纳入。60 秒是可配置的
扫描间隔，不是失联阈值或告警送达时限；任务从本轮结束后计时，实际执行还受
Worker、Store 和周期任务租约影响。

没有成功记录，或距最近成功上报超过对应阈值时，生成通知。默认阈值如下：
...
| `NVIDIA_KERNEL` / `FABRIC_MANAGER_LOG` / `NODE_LOGS`（纳入检查时） | 900 秒 |

控制面写入 `AdvisoryNotification` 并通过现有 Notification Dispatcher（SNS 或 SES）
投递，按集群、节点和通道的默认 3600 秒时间桶去重。该扫描不向节点发探测请求，
不写统一的 `UNKNOWN` 节点状态，也不因此创建恢复 workflow。
Agent 心跳租约已过期的节点不参与本扫描及对应指标统计，应另看 Fleet readiness 和
stale-agent 指标；不能将“没有采集器静默告警”等同于“节点健康”。

没有 AMP 的 region 设置
`GPU_FAULT_ENABLE_AMP=false`，安装脚本跳过 AMP/ADOT 资源，内置通知路径仍生效。
...
| ② 关联 | Completion Service | 处理唯一训练终态：关联可信 marker；无 marker 时在重启预算内单次重启；allocation 缺失时取证并升级 |
...
| ⑤ 验证与留痕 | Notification Service | 生成并可选发送 advisory 通知（SNS 主题或 SES 邮件） |
...
Schema 由**版本化迁移框架**维护（当前 14 个版本；v7 增加 Processor 延迟重试与
lane policy，v13 增加控制面评审索引与 autovacuum 参数，v14 增加 `gpu_fault_objects`
的 wakeup NOTIFY 触发器，导入期校验注册表连续性与 DDL 源码校验和），生产部署用
...
下图概括区域生产形态下的主动故障主流程：采集信号异步入队，控制面在后台判定；
需要处置且满足执行条件时，由受 lease/fencing 保护的 workflow 推进获准步骤。

```mermaid
sequenceDiagram
    autonumber
    participant C as GPU 采集器
    participant P as CPU 控制面
    participant E as GPU 集群执行器
    participant T as 执行目标
    C->>P: 上报故障信号
    P-->>C: 202 已入队（不代表修复完成）
    P->>P: 后台判定、编排<br/>检查执行条件
    loop 每个获准的远端步骤
        E->>P: 领取本集群命令
        P-->>E: 返回授权命令
        E->>T: 校验后执行或查询
        T-->>E: 动作结果或仍在处理
        E->>P: 回报成功 / 等待 / 失败
        P->>P: 保存进度<br/>推进 / 等待 / 转人工
    end
    P->>P: 达到终态后<br/>保存结果并通知
```

“CPU 控制面”合并了 API、规则、编排和调度角色；“执行目标”按步骤对应 Kubernetes、
Node Agent 或 HyperPod，并非每步都调用三者。图中省略控制面本地的取证、样本验证等步骤。

- **受理不等于处置完成**：`202` 只确认入队，后台消费无需等待客户端收到回执。
  无需动作、复用已有处置、等待证据或 `BLOCKED` 时，不会直接走完整条修复流程。
- **集群执行器主动拉取**：CPU 不直接操作 GPU 节点；本地 Adapter 经校验后执行，
  Node Agent 请求必须签名。安全处置模式仅运行获准的安全步骤。
- **进度与收尾分开提交**：普通进度通常只保存 workflow；终态按 incident 当前归属
  决定是否联合提交，通知随后处理。单条 workflow 终态不一定代表后继处置链已结束。

参与者与部署组件的对应关系、完整分支和存储边界见
[详细设计 §1.3](详细设计.md#proactive-fault-sequence)。
...
```mermaid
flowchart TD
    A["Watcher 观察受管训练<br/>记录 rank、节点和 GPU"] --> B["满足终态条件<br/>上报唯一训练终态"]
    A -. "发现失败" .-> C["保全证据<br/>停止整个训练 attempt"]
    C -. "异步停止，不阻塞上报" .-> B
    B --> D{"需要被动恢复？"}
    D -- "正常成功 / 用户停止<br/>或其他 incident 发起终止" --> X["不新建恢复<br/>结束或沿用原流程"]
    D -- "失败 / 超时<br/>或本系统遏制造成的停止" --> E{"有可用故障标记？"}
    E -- "有" --> F["复用已有恢复流程<br/>或按标记生成处置计划"]
    E -- "无" --> H{"有分配记录<br/>且 Profile 允许重启？"}
    H -- "否" --> I["取证并通知运维<br/>不自动重启"]
    H -- "是" --> J["生成单次训练重启计划"]
    F --> G["执行安全门禁与处置<br/>校验重启预算和修复状态"]
    J --> G
    G --> L["回写处置结果<br/>若重启，创建新 attempt"]
```

图中虚线是异步遏制路径，不是终态上报的同步前置条件。完整分支见
[发现失败与遏制](详细设计.md#passive-failure-containment)、
[终态决策与恢复](详细设计.md#passive-terminal-decision)。

1. 首次确认关键 rank 非零退出，或 workload adapter 报告 `FAILED` 时，产生一次
   FailureDetected；主动删除或其他 incident 发起的终止不按普通训练失败递归处理。
...
4. 只有 failure 事件持续未被控制面受理超过配置超时，Watcher 才执行带
   `passive-stop-mode=emergency-fallback` 标记的紧急 suspend。
5. 全部关键 rank 结束，或失败后的 cleanup timeout 到期等终态条件满足时，
   产生唯一 TerminalEvent；投递失败由 outbox 重试。
6. 终态可以先于 FailureDetected 到达。满足条件时由 Completion Service 幂等补建
   遏制，再立即作出决策；恢复 workflow 的前驱依赖保证执行顺序，终态上报不等待停训练完成。

拿到唯一终态后，先区分停止来源，再决定后续动作：

| 终态情形 | 处理 |
|---|---|
| 其他 incident 发起终止，或显式 initiator 无法确认 | `NO_ACTION`，不递归创建恢复 |
| 正常成功，或用户主动停止，且无本 attempt 被动遏制 | 不自动恢复，并撤回该 job 的相关重启 workflow |
| 本 attempt 被动遏制造成的 `STOPPED` | 继续后续恢复判定；即使 initiator 标注丢失，也查询已存的被动遏制记录 |
| 需要恢复，且 allocation 范围内有可用的可信 marker | 选择动作优先级最高者；已有 workflow 负责重启时不重复恢复，否则等待对应 incident 恢复，或按 marker 生成处置计划 |
| 需要恢复，无可用 marker 但 allocation 存在 | 不做硬件定因，依据已有 allocation 规划一次 `RESTART_WORKLOAD`，次数受同一 job 重启预算约束；Profile 不能重启时改为取证并升级运维（`no-hardware-evidence:RESTART` / `:ESCALATE`） |
| 需要恢复，且 allocation 缺失 | 取证并升级人工（`allocation-missing:INCONCLUSIVE`）；不猜测节点，不自动重启训练，也不据此虚构节点隔离动作 |

“正常成功”还要求退出证据不指向失败；具体判定以 `TerminalEvent.is_failure` 为准。
“allocation 存在”只表示列表非空，不等于完整性已经验证。“可用 marker”的过滤规则见
[详细决策](详细设计.md#passive-terminal-decision)。计划创建不等于允许重启：
执行期仍检查前驱、授权、incident 前提和预算；预算耗尽只禁止重启，已规划的其余
节点修复步骤继续，最后 workflow 以 `FAILED` 收尾。

### 5.3 工作流执行门禁

工作流状态为 `PENDING`/`SAFETY_PENDING`/`BLOCKED`/`RUNNING`/`SUCCEEDED`/`FAILED`/
`SUPERSEDED`；后三者与 `BLOCKED` 都不会再被 dispatcher 执行，`SUPERSEDED` 由
`workflow-reconcile` 写入，表示该记录的目标已被后续 workflow 恢复。
...
`hardware-inventory-mismatch-zh-v1` 通知，并执行：
...
重复回执不产生第二次副作用，包括不产生第二次通知投递。协议的模块级设计见详细设计
...
| RegionalClusterRegistration | `cluster_id` | 集群注册、token 摘要（轮换期另含 retiring 摘要与过期时刻）与 namespace allowlist |
...
`create_app()` 当前挂载 92 条应用路由（91 个路径）；区域生产形态会在其上安装五桶默认拒绝
中间件。按功能分为：
...
- `/v1/regional/registry/*`：不可变 revision 发布、收敛状态与单调 rollback。收敛以发布时
  活跃的控制面成员集合为准，超过 `GPU_FAULT_REGISTRY_STALE_SECONDS`（生产 90 s）
  无心跳的成员视为离开 fleet，不阻塞收敛。
...
- `/v1/regional/*`：集群注册查询、远端命令领取/续租/进度/回执、HyperPod 提交幂等、
  incident 归属与 fleet rollout fence 查询、spare 健康、Agent drain/revoke、证据与
  通知回写，共 **16 个端点**（另 4 个在 `/v1/regional/registry/*`）。路由在本地
...
2/1/38/7/44（`public` 为 `/healthz` 与 `/livez`）。新增路由若未声明桶，应用装配期
即启动失败；运行期取不到桶时，路径无任何路由服务返回 404，路由存在却未声明桶
一律 403（fail-closed）。
模型细节见详细设计 §4.5。
...
两侧组件、连接方向、监听端口与凭证的交互拓扑见 §4.1.1；
已启用组件与 AWS 服务的核对快照见 [§4.1.2](#deployment-architecture-snapshot)。

每 GPU 集群侧部署 §4.2 表中标注"GPU 集群"的全部组件，含 Cluster Action Executor。
集群注册的启动快照来自 `GPU_FAULT_REGIONAL_CLUSTERS_JSON` Secret，运行时以 Aurora 中的
durable registry head 为准（join-cluster/remove-cluster 在此发布不可变 revision，Secret
与 head 不一致会报 `secret_drift`）；区域模式不接受单值 `GPU_FAULT_HYPERPOD_CLUSTER`。

管理员通知由区域控制面发送：控制面 EKS ServiceAccount 的 Pod Identity role 按站点渠道
持有站点 SNS 主题的 `sns:Publish`（`channel: ses` 时为限定到已验证 sender 的
`ses:SendEmail`）；GPU 集群 executor 两者都不需要。通知走 Aurora outbox——业务处理只
创建幂等 notification，dispatcher 用 owner/epoch/lease claim 并指数退避；旧 epoch 不能
完成新 lease，SNS/SES 故障不能反向改变 workflow step 结果。

部署与纳管由 `gpu-fault-admin` 驱动（`deploy` 一条命令覆盖首装、升级、加集群、单步回滚
与 profile 计划批准；另有 `join-cluster`/`remove-cluster`/`uninstall`/`rotate-token`/
`config`/`status`），底层是 `src/gpu_fault_release` 的发布引擎：每次发布是一个可恢复
事务（`PREPARING → CPU_STAGED → ROLLING_CLUSTERS → FINALIZING → COMMITTED`，失败为
`FAILED`/`PAUSED → ROLLING_BACK → ROLLED_BACK`），控制面一次滚一个副本、GPU 集群按
`upgradeMaxParallelClusters` 逐个滚，验证或稳定窗口失败时按 `spec.autoRollback` 自动
回滚到上一发布；跨 schema 版本的发布见 §11 第 4 条。发布工件（wheel、node bundle）以
ConfigMap 承载，wheel 以 `.xz` 压缩存储。状态都在 `--state-dir` 下（site.yaml、
checkpoint、join/rotate 的 state.json）。
...
不同 GPU 集群不得共享 Token，Token 只保存在对应集群 Secret（及站点 `--state-dir` 的
token 文件）中，经 `gpu-fault-admin rotate-token` 轮换。数据面必须校验控制面证书链与
主机名；禁止 `insecure`/跳过校验开关。
...
在此之前，发现 Token 泄露用 `gpu-fault-admin rotate-token --gpu-cluster-arn …
--window-minutes 10` 立即轮换：注册表进入新旧摘要并存的重叠窗口，数据面与节点 Agent
分波切换，窗口关闭后旧摘要失效；随后审计该 cluster_id 的全部事件。

### 8.2 授权模型

- 区域模式授权默认拒绝，且不靠路径前缀：每条路由用装饰器显式声明五个桶之一
  （`public`、`metrics`、`cluster-token`、`dual-credential`、`execution-token`），
  应用装配期遍历全部路由，任一条未声明即启动失败；运行期取不到桶时，路径无任何
  路由服务返回 404，路由存在却未声明桶一律 403（fail-closed）。
...
- Kubernetes RBAC 只给到动作真正需要的动词：`nodes` 为
  ...
  这些写动词不在 ClusterRole 里：ClusterRole 只授 `nodes` patch 与各 kind 的读动词，
  写动词由发布引擎按每个 allowed workload namespace 渲染 Role/RoleBinding（executor：
  `pods` patch/delete、训练对象 create/patch；Completion Watcher：`pods` patch、
  `pods/exec` get/create、`pods/log` get、训练对象 patch），device plugin 的
  `pods delete` 只在 `kube-system`。namespace 之外的命令被 executor 与 API server
  双重拒绝。
...
- `sns:Publish`/`ses:SendEmail` 只授予控制面侧 ServiceAccount，GPU 集群侧不授予。
...
- 已关闭的 incident 及其 workflow 默认保留 30 天（`spec.retention.controlRecordRetentionDays`，
  默认开启，显式写 0 关闭），到期先归档到站点 S3 桶再删除。
...
- 启用通知时，XID/SXID notification result 为 `SENT` 且 `provider_message_id` 非空
  （SNS 或 SES 的 MessageId）；`GPU_FAULT_ALLOW_EMAIL=true` 但 IAM 返回 AccessDenied
  必须视为上线失败。
- `RESET_GPU` 和 `RESTART_WORKLOAD` 各有一条成功通知；重复 remote result 不得产生
  第二次 SNS/SES 投递。低利用率通知必须按节点/任务聚合，不能按 GPU 数量线性增长。
...
2. 通用 Kubernetes 部署当前未实现为交付能力；遗留清单含 `REPLACE_WITH_*`，
   只作开发参考，不能原样上线。
3. 若直接使用部分 HyperPod 示例清单，必须替换其中的测试集群名、节点名、Service 名和
   wheel ConfigMap 名；推荐使用 `gpu-fault-admin deploy` 由发布引擎渲染；
   `deploy/hyperpod/deploy.sh` 只服务 §7.2 的历史单集群形态。
4. PostgreSQL schema 已有版本化迁移框架（当前 14 个版本，v7 增加 Processor
   `not_before`、`retry_count`、`lane_policy` 与可用性索引，v13 增加控制面评审索引与
   autovacuum 参数，v14 增加 `gpu_fault_objects` 的 wakeup NOTIFY 触发器；导入期校验
   版本连续、名称唯一和 DDL 源码校验和，生产用 `gpu-fault-store-migrate --ensure-schema`
   配合 `deploy/migrations/` 的 Job 单独执行，业务进程不自建表）。残留限制是
   **只向前**：没有 down migration，且业务进程只接受与自身一致的 schema 版本，跨版本
   发布不能回滚。这类发布必须以 `gpu-fault-admin deploy --accept-schema-change` 执行：
   发布引擎先创建 Aurora 手动快照，再把本事务标为 fail-forward（验证失败不自动回滚，
   `SKIPPED_POLICY`），`spec.autoRollback` 不被改写；唯一的后退路径是恢复快照后再回滚。
...
10. **只支持 HyperPod EKS 编排，HyperPod Slurm 编排暂不支持**（§2.1 约束 4）。
    ...
    stop/restart 执行器；`Environment` 枚举中的 `hyperpod-slurm`
