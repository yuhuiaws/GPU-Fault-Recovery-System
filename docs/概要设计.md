...
Schema 由**版本化迁移框架**维护（6 个版本，导入期校验注册表连续性与 DDL 源码
校验和），生产部署用 `gpu-fault-store-migrate --ensure-schema` 单独执行，业务进程
...
- `/v1/regional/*`：集群注册查询、远端命令领取/续租/回执、HyperPod 提交幂等、
  spare 健康、Agent drain/revoke、证据与通知回写。仅区域形态启用，
  共 **14 个端点**，逐个见详细设计 §4.4。
...
1/1/36/7/36。新增路由若未声明桶，应用装配期即启动失败；运行期取不到桶一律 403。
模型细节见详细设计 §4.5。
...
- 容量必须按"每进程值 × 每Pod进程数 × 副本数"折算，不能按Pod数折算：后台线程和
  ...
  6 × 4 × 24 = 576。同理，`--limit-max-requests` 之类的每进程限额要用
  "限额 ÷ 间隔 vs. 产生速率"核对，否则周期性回收会撞上后台循环。
...
4. PostgreSQL schema 已有版本化迁移框架（当前 6 个版本，导入期校验版本连续、
