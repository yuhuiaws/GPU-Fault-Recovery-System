...
| 代码版本 | 0.10.0 |
| 事实核对日期 | 2026-09-15（以当前 `main` 的源码、生成清单和测试契约为准） |
...
lease 已过期的节点不参与指标或通知告警，避免已退役节点形成永久静默误报：
...
超时后控制面直接写 `AdvisoryNotification` 并通过现有 Notification Dispatcher（SNS
或 SES）发送；同一节点、collector 和 channel 每小时最多一封。没有 AMP 的 region 设置
