...
Planner 只表达动作与目标节点；`RESTART_WORKLOAD` 的执行参数（cluster/job/attempt/GPU 数/预算、avoid_node_ids、`requires_incident_state`）全部由 Passive Compiler 依据 `RecoveryPlan.avoid_node_ids` 与 `RecoveryPlan.restart_after_incident_id` 推导。`SimulatedRecoveryExecutor` 在模拟前同样从 plan 字段读取这些前提。重启配额只在控制面预检 `reserve_restart_budgets` 预留；下发时由 `issue_restart_authorization` 读回预留并签发 `RestartAuthorization`，数据面 `_restart_guard` 只比对授权、incident 前提、避让节点与 GPU 数。预算耗尽因此是 claim 时的预检结论（workflow 直接 `FAILED`，不调用任何 adapter），耗尽邮件也由预检 `fail_restart_preflight` 发出，而不是 Kubernetes Adapter。
