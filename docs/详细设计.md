...
`watcher.py` 提供 runtime-neutral 状态机；Kubernetes 侧的实现拆成六个模块（F6b 纯搬移）：
`completion_controller.py` 只保留 `KubernetesCompletionController` 宿主类（reconcile pass、
`resume_tombstoned_attempt`）、`controller_from_environment` 与 console script 入口 `main`，其余
按职责落在 `completion_pod_parsing.py`（label/annotation 常量、Pod → Observation 解析）、
...
failure containment、逾期紧急停止）和 `completion_reconcile_loop.py`（list/watch 循环、
`run_watch_cycle` 去抖）。日志名统一借用 `gpu_fault.completion_controller`，所以拆分前后的
日志行逐字节相同。
...
原单文件 `cluster_executor.py` 已按层拆成包（F6a 纯搬移，`__init__` 原样再导出 23 个名字，
`gpu_fault.cluster_executor:main` / `:readiness_probe` 两个 console script 不变）：
...
`SpareReservationSweep`）与 `bootstrap.py`（`executor_from_environment`、`readiness_probe`、`main`）。
...
### 4.2 集群侧事件入口（`cluster-token`，共 35）

以下 22 条是采集侧和节点侧入口，全部要求 `X-GPU-Fault-Cluster-ID` 与匹配的每集群
