...
1. 校验 profile；若 attempt 尚无被动遏制 incident、终态为 `FAILED/TIMED_OUT`、
...
2. 在 event_key 事务内保存 event、决策与计划。
3. 恢复 workflow 以 `predecessor_workflow_id` 指向遏制 workflow，由调度器保证先停后启；
   终态上报不再返回 409、不再等待遏制持久化。
4. 由其他 incident 发起的终止（`termination_initiator_incident_id` 指向非本 attempt
   被动遏制 incident）→ `NO_ACTION`，不递归恢复。
5. 用户 `STOPPED/SUCCEEDED` → `NO_ACTION` 并撤回该 job 的重启 workflow；存在本 attempt
   被动遏制 incident 的 `STOPPED`（无论是否带 initiator，DESTR-015 tombstone 竞争会丢掉
   该标注）继续恢复。
6. 在终态前后 10 分钟窗口内匹配可信、active、未过期且 scope 与 allocation 相交的 marker。
   ...
   作业恢复**——它只观察节点、不修节点，terminal 照常走无证据重启决策；marker 本身保持 active。
   没有存到 incident 的诊断 marker 仍按 `from_marker` 编出它要求的诊断计划。
7. 有 marker 时选择动作优先级最高者。
8. 已有主动 workflow 拥有 workload restart 时不创建第二次恢复。
...
Planner 只表达动作与目标节点；`RESTART_WORKLOAD` 的执行参数（cluster/job/attempt/GPU 数/预算、avoid_node_ids、`requires_incident_state`）全部由 Passive Compiler 依据 `RecoveryPlan.avoid_node_ids` 与 `RecoveryPlan.restart_after_incident_id` 推导。`SimulatedRecoveryExecutor` 在模拟前同样从 plan 字段读取这些前提。重启配额只在控制面预检 `reserve_restart_budgets` 预留；下发时由 `issue_restart_authorization` 读回预留并签发 `RestartAuthorization`，数据面 `_restart_guard` 只比对授权、incident 前提、避让节点与 GPU 数。预算耗尽因此是 claim 时的预检结论（workflow 直接 `FAILED`，不调用任何 adapter），耗尽邮件也由预检 `fail_restart_preflight` 发出，而不是 Kubernetes Adapter。每个计划只有一个 incident 前提：无证据重启与 marker 计划以本 attempt 的被动遏制 incident 为前提（遏制 STOP 失败 → incident `ESCALATED` → 守卫拒绝重启）；`after_incident` 计划仍只以其节点 incident 为前提，遏制结果不额外门控。
