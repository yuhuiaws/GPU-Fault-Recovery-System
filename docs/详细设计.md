...
| 改数据库表或迁移 | §3.1 表结构 → §3.2 迁移 → §3.3 保留与清理 |
...
本文对应 `gpu-fault-control-plane 0.10.0`。包入口如下：
...
为可重放，下一次投递前最多重放10条；400/403/413等永久拒绝标记为不可重放
dead-letter供审计。默认每个文件最多1000条，路径为
`/var/lib/gpu-fault/outbox/<collector>.ndjson`。
...
文件首次tail从完整行开始，ISO时间戳作为`observed_at`；state临时文件和父目录都执行
`fsync`。systemd instance unit（例如`nvidia-fabricmanager@0.service`）属于合法来源。
...
Node Agent 的无鉴权 `/healthz` 只返回 `{"status":"ok"}`；能力、节点身份和 mutation
...
`min_length=max_length=64`，且必须是合法十六进制）、`enabled`（默认 `True`）、
`allowed_namespaces`、`created_at`、`updated_at`。

认证走 `authenticates(token)`：对入参取 SHA-256，与 `token_sha256` 用
...
`GPU_FAULT_PROCESSOR_OBSERVATION_WORKERS=2`，GPU/Host 遥测池取默认 6。
...
让采集侧重试，故障流量宁可排队也不能丢。四个值任一非正即启动失败。
...
claim 在同一事务中使用 `FOR UPDATE SKIP LOCKED` 锁定该 lane 最早的请求，再获取或
接管 lane lease；完成和释放同时校验 request 与 lane 的 token/epoch。旧 owner 失联
后，新 consumer 在 lease 到期后接管，旧 owner 不能提交结果。该设计没有预分配的
cluster partition，也没有单 processor leader 吞吐瓶颈。表结构见 §3.1.4。
...
`/opt/gpu-fault/venv/bin/py-spy` 的固定 SHA-256；版本或二进制不匹配会使节点安装失败。
...
Store 对外提供事件、profile、marker、decision、diagnostic、plan、incident、workflow、Agent、fleet、barrier、指标 baseline、finding、collector status、evidence 和 training progress 的持久化接口。
...
| 通用对象 | `gpu_fault_objects` | `PRIMARY KEY(kind, key)`；`payload JSONB NOT NULL` | 通用领域对象存储。incident、workflow、notification、event、marker、evidence、fleet、regional command 等通过 `kind` 区分。 |
...
- Fleet/区域执行：`agent`、`hyperpod_node_identity`、`hyperpod_submission`、
  `regional_cluster`、`remote_command`。
...
`create_postgres_schema()` 当前创建 50 个 `CREATE INDEX IF NOT EXISTS`
（47 个来自 `ddl.py`、3 个来自 `ddl_spool.py`）、6 个 plpgsql 函数和 8 个触发器：
...
`POSTGRES_SCHEMA_MIGRATIONS` 当前有 **6 个版本**，`LATEST_POSTGRES_SCHEMA_VERSION`
...
| `_validate_existing_schema()` | `GPU_FAULT_POSTGRES_AUTO_SCHEMA_INIT=false`，即三层控制面全部生产进程 | 只读校验：10 张关键 relation、3 个 plpgsql 函数、2 个 notify 触发器、3 个优先级计数触发器都存在；迁移历史逐行等于代码内注册表；`gpu_fault_schema_version.version` 等于 `LATEST_POSTGRES_SCHEMA_VERSION`。任一不符抛 `RuntimeError`，提示先跑 `--ensure-schema` |
...
区域模式（`GPU_FAULT_DEPLOYMENT_MODE=regional`）下真实注册 **81 条路由**，按实现
模块分布：
...
| `src/gpu_fault/app/routes/configuration.py` | 2 |
| `src/gpu_fault/app/routes/processor.py` | 2 |
| `src/gpu_fault/app/metrics.py` | 2 |

授权桶分布：`execution-token` 36、`cluster-token` 36、`dual-credential` 7、
...
双凭据（`dual-credential`，7 条）——带 `X-GPU-Fault-Cluster-ID` 时按每集群 token
判定，否则要 operator execution token：
...
仅 `GPU_FAULT_DEPLOYMENT_MODE=regional` 时注册，共 **14 个端点**，全部在
`src/gpu_fault/app/routes/regional.py`：
...
`GET /v1/regional/clusters` 返回注册列表，`token_sha256` 脱敏。
...
| `dual-credential` | 带集群头按 `cluster-token`，否则按 `execution-token` |
...
`GET /metrics` 输出 Prometheus 文本，约 140 个指标族（`# HELP` 计，随后端和开关
浮动），集中在四类：

| 前缀 | 覆盖内容 |
|---|---|
| `gpu_fault_processor_*` | 队列深度、claim 轮次/命中/空转、lane 阻塞、admission 批与拒绝、租约与超时、计数模式 |
| `gpu_fault_ingress_*` | 每 lane 在飞数、等待时长、背压拒绝 |
| `gpu_fault_telemetry_spool_*` | spool 深度、合并/取代、租约字节、重放耗时与传输结果、通知回退 |
| `gpu_fault_store_io_*`、`gpu_fault_request_decode_*`、`gpu_fault_event_loop_lag_seconds`、`gpu_fault_postgres_pool_checkout_wait_seconds` | 进程内 store I/O 与解码准入、事件循环滞后、连接池等待 |

**尚未导出的指标**：workflow outcome、dispatcher backlog、通知投递结果、
stale Agent、collector freshness 都只在 Store 里有记录，没有对应 metric。判断这些
维度只能查 API 或数据库；把"`/metrics` 干净"当成"通知和工作流正常"是错误推论。
API 延迟同样没有直方图，只有事件循环滞后可作间接信号。

## 8. 测试设计

`tests/` 共 136 个测试文件，规模最大的四组是 `tests/regional/`（28）、顶层
`tests/`（21）、`tests/orchestration/`（12）、`tests/hyperpod/`（11），其余覆盖
execution、node_agent、collectors、store、processor、metrics、completion 与
host_health。策略、XID/SXID、分布式 reset、API、Watcher、Completion、Executor、
PostgreSQL、Fleet、Agent、HMA、指标、证据与通知都在其中。

`make check` 是提交门禁，串联格式与静态检查、`compileall`、11 个
`scripts/check-*.py` 结构守卫、`make docs-check`、清单与 YAML/Shell 检查、
`make artifact-check`（重建 wheel 并证明它就是当前源码），最后跑全量 `pytest`。
纯文档改动也必须跑 `make docs-check`。`make coverage` 用 4 个 worker 跑同一套测试
并强制 78% 覆盖率下限，且**不**启用外部 PostgreSQL；带真实库的用例走
`GPU_FAULT_TEST_POSTGRES_URL` + `make test-postgres`。

`make docs-check` 本身是六个生成物/引用一致性检查
（doc-impact、env-doc、case-index、manual-command-order、doc-reference、
fault-evidence）加 9 个文档契约测试文件。生成类文档
...
2. `make test-postgres`（真实 PostgreSQL 的 store/claim/重连契约）；
3. simulation 模式的 API 与策略验证；
4. 节点 `/opt/gpu-fault/verify`；
5. 区域形态非破坏性 passive closed-loop E2E；
6. 维护窗口内逐项启用 reset/reboot，并验证 fencing 和回滚（replace 恒禁用，
   见概要设计 §2.1）。
