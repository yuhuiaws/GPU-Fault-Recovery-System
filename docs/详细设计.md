...
Planner 只表达动作与目标节点；`RESTART_WORKLOAD` 的执行参数（cluster/job/attempt/GPU 数/预算、avoid_node_ids、`requires_incident_state`）全部由 Passive Compiler 依据 `RecoveryPlan.avoid_node_ids` 与 `RecoveryPlan.restart_after_incident_id` 推导。`SimulatedRecoveryExecutor` 在模拟前同样从 plan 字段读取这些前提。
