...
| 改恢复动作或 workflow 编排 | §2.3.1 动作阶梯 → §2.9 workflow → §2.11 adapter |
| 改数据库表或迁移 | §3.1 表结构 → §3.3 迁移与一致性 → §3.2 保留与清理 |
| 改 API 或鉴权 | §4 API 设计（§4.5 鉴权分桶与 loopback 身份） |
| 改部署、集群加入/移除或卸载 | §2.2 能力编译与清单 → §2.22 安装资源注册表 → §2.23 管理员 CLI → 《开发者部署实现》 |
...
flowchart TB
    ...
    subgraph CP["区域控制面（唯一生产形态，见 §2.18）"]
        ...
        ADP["Adapter 层 §2.11<br/>K8s / HyperPod / Node Action"]
        NOTIF["通知 §2.11、§7<br/>固定模板、去重"]
...
部署机上的管理员 CLI 与发布内核（§2.23）不在运行图内：它经 §4 的 execution-token 路由和
Kubernetes/AWS API 变更上面四层，自身不参与故障判定。

两点容易看错的地方：**判定只发生在控制面**，数据面的 collector 和 Node Agent 都
不做策略判断（Node Agent 甚至不知道自己为什么被要求重启）；**控制面 Adapter 不直接连
节点**，所有落到节点上的动作都要经过 §2.16 的远程命令协议由集群内 executor 拉取，
再由 executor 内的本地 Adapter 调用执行目标。

### 1.3 一次故障的完整生命周期

<a id="proactive-fault-sequence"></a>

下图以区域生产形态下 kernel 日志触发的 XID 处理为例，展开异步受理、判定、调度、
远端执行和收尾。它不是固定的“XID → 节点重启 → 训练恢复”模板；是否需要动作、
采用哪条分支均由证据和策略决定。metrics、host log 等信号有各自的 Finding 入口，
不应把它们与 XID 判定视为同一种请求模型。主流程简图见
[概要设计 §5.1](概要设计.md#proactive-fault-overview)。
...
sequenceDiagram
    ...
    participant W as CPU Worker<br/>Processor / 规则 / 编排
    participant S as Aurora<br/>队列与状态库
    participant D as CPU Dispatcher<br/>工作流引擎
    participant E as GPU Cluster Executor<br/>本地 Adapter
    participant T as Kubernetes / Node Agent<br/>或 HyperPod

    C->>I: 上报 kernel 故障日志
    I->>I: 鉴权、集群绑定、准入校验
    I->>S: 持久化 ProcessorRequest
    S-->>I: 入队成功
    I-->>C: HTTP 202 + request_id<br/>只确认入队，不代表处置完成
    Note over I,W: 入队回执与 worker 消费异步进行<br/>消费不以客户端收到 202 为前提
    W->>S: 按优先级与 lane 租约领取请求
    W->>W: 本地回放业务路由<br/>规范化、关联证据与策略判定
    alt 等待伴随证据
        W->>S: 保存 PENDING_CORRELATION 与关联截止时间
        Note over W,S: 本次不编排动作；周期关联器到期后再判定<br/>原 Processor 请求可先完成
    else 已能作出处置判定
        W->>W: 代际检查、事件去重与工作流仲裁
        alt 无需新动作 / 复用已有处置
            W->>S: 记录 NO_ACTION 或关联已有 incident/workflow
        else 可以执行正式处置
            W->>S: 原子创建 incident + workflow(PENDING) + 事件索引
        else 仅能安全处置 / 缺少执行条件
            W->>S: 创建 SAFETY_PENDING 安全工作流<br/>或 BLOCKED 记录
        end
    end
    W-->>D: 唤醒调度（另有周期扫描兜底）
    W->>S: 保存 Processor 处理回执<br/>事件处理完成不等于 workflow 完成
    Note over W,D: marker、策略结果和通知按各自路径持久化<br/>不属于一笔贯穿全流程的大事务

    opt 存在可执行 workflow
        D->>S: 检查前驱与节点冲突，领取 workflow 租约及资源预算
        D->>D: claim 后预检重启预算<br/>耗尽时只扣留重启步骤
        Note over D,E: 可调度 PENDING / RUNNING / SAFETY_PENDING<br/>SAFETY_PENDING 或 safety_only 仅执行安全步骤；BLOCKED 不执行
        loop 每个就绪步骤或可合并批次
            D->>D: 校验 fencing、权限与步骤前提
            alt 控制面本地步骤
                D->>D: 本地 Adapter 执行取证、样本验证或支持升级
            else GPU 集群远端步骤
                D->>S: RegionalRemoteWorkflowAdapter<br/>幂等创建/复用 RemoteActionCommand
                Note over D,S: 新命令尚未完成时，代理返回 WAITING<br/>不是 CPU 直接操作 GPU 节点
                E->>I: 通过 API 领取本集群命令
                I->>S: 原子领取命令租约，校验集群与版本
                I-->>E: 命令、租约、fencing 与执行上下文
                E->>E: 校验 Fleet、作用域、唯一 Adapter 与 barrier
                E->>T: 本地 Adapter 执行或查询动作<br/>Node Agent 请求须签名
                T-->>E: 已完成结果或仍在处理
                E->>I: 上报 SUCCEEDED / WAITING / FAILED
                I->>S: 保存远端回执（独立于 workflow 进度提交）
                D->>S: 读取已落库的远端结果
            end
            alt WAITING
                D->>S: 按租约保存 workflow 内的等待进度与步骤记录
                D->>D: 挂起依赖步骤，在时限内继续等待
            else FAILED
                D->>D: 按失败类别补偿、升级分支/后继或转人工
                D->>S: 保存失败/补偿进度<br/>需终态化时走条件联合提交
            else SUCCEEDED
                alt 成功步骤带节点重绑定
                    D->>S: 按当前归属条件联合保存 workflow / incident
                else 普通进度
                    D->>S: 按租约保存步骤进度<br/>普通进度通常只更新 workflow
                end
                D->>D: 推进后续步骤或 DAG 分支
            end
        end
        opt workflow 达到终态
            D->>S: 按归属条件联合保存 workflow / incident
            Note over D,S: incident 已由后继接管时，只保存原 workflow<br/>步骤记录属于 workflow，不是独立原子更新的第三张表
        end
        opt 终态已提交
            D->>S: 终态钩子退役适用 marker、记录通知等
        end
    end
```

图 1-2 主动故障从异步受理到工作流收尾的详细时序

图中参与者是职责划分，不是一列对应一个独立 Deployment：

| 参与者 | 对应组件或实现 | 详见 |
| --- | --- | --- |
| GPU Collector | GPU 节点上的 kernel 日志采集器 | §2.5 |
| CPU API / Ingress | `gpu-fault-api-ha`，负责鉴权、准入、入队和远端命令 API | §4、§2.18 |
| CPU Worker | `gpu-fault-control-worker` 内的 `ProcessorCoordinator`、业务处理器、`GpuFaultPolicyEngine` 和 `IncidentOrchestrator` | §2.3、§2.9、§2.18.3 |
| Aurora | 控制面 `PostgresStore` 使用的 Aurora PostgreSQL，保存队列、workflow 和远端命令；不是节点侧 SQLite ledger | §3 |
| CPU Dispatcher | 同一 `gpu-fault-control-worker` 内的 `WorkflowDispatcher` 与 `ProductionWorkflowExecutor`，不是额外 Deployment | §2.9、§2.18 |
| GPU Cluster Executor | 目标 GPU 集群内的 `gpu-fault-cluster-executor` 及其本地 Adapter | §2.11、§2.17 |
| Kubernetes / Node Agent / HyperPod | 按 operation 选择的实际执行目标，不是一个统一服务，也不是每步依次调用三者 | §2.11、§2.12、§2.16 |

关键语义与实现依据：

1. **三个完成点不同**：Ingress 的 `202` 只确认入队；Processor 处理回执只确认本次
   事件处理；workflow 终态才表示该条工作流结束，后继工作流仍可能继续。
   `PENDING_CORRELATION` 可先结束本次请求，等待关联器再判定。依据为
   `app/middleware/dispatch.py::dispatch_processor_request`、
   `processor/coordinator.py::ProcessorCoordinator._execute` 和
   `app/ingest/faults.py::FaultIngestionService.ingest_xid`（均在 `src/gpu_fault/` 下）。
2. **判定不必产生新动作**：事件去重、复用、`NO_ACTION`、安全处置和 `BLOCKED`
   不能合并为“总会创建并执行正式 workflow”。`safety_only` 在状态进入 `RUNNING`
   后仍约束可执行步骤；不能只看状态名判断权限。依据为
   `IncidentOrchestrator.ingest`、`IncidentOrchestrator._build_workflow`
   和 `workflow.executes_safety_steps`。
3. **Adapter 分为代理和实际执行者**：控制面
   `RegionalRemoteWorkflowAdapter.execute` 创建或复用命令，未完成时返回 `WAITING`；
   集群 Executor 通过 API 领取命令，再由 `CommandDispatch.execute` 选择本地 Adapter。
   Executor 不直连 Aurora，CPU 不持有 GPU kubeconfig。
4. **不存在贯穿全链的单笔事务**：新建 incident、workflow 和事件索引可原子提交；
   远端回执、普通 workflow 进度、marker 和通知各有提交边界。
   `ProductionWorkflowExecutor._save_leased` 通常只保存 workflow；
   `_save_leased_state` 和 `_save_terminal` 仅在 incident 仍归属于当前 workflow
   时联合保存二者，否则只保存原 workflow，避免覆盖后继处置。
   步骤记录嵌在 workflow 中，终态钩子在终态提交后运行。
5. **失败不是无条件升级**：按失败类别进入补偿、升级分支、后继 workflow 或人工处理；
   缺少执行条件时 fail closed，不保证自动隔离或自动恢复。合法重启预算耗尽时只扣留
   重启步骤，其他获准修复步骤仍可继续；身份或上下文校验失败则不能套用这一继续执行规则。

### 1.4 版本与包入口

本文对应 `gpu-fault-control-plane 0.10.0`，最近一次按当前 `main` 源码、生成清单
和测试契约逐项核对的日期为 **2026-09-15**。包入口如下：
...
共享 Store 和鉴权边界。部署机上的管理员 CLI 与发布内核单列在 §2.23。
...
- Environment：`eks/hyperpod-eks/hyperpod-slurm/kubernetes` 四值；裸 `ec2`/`slurm` 不再是合法
  取值，解析期直接拒绝——没有任何判定分支消费它们，一个能加载却静默走默认路径的取值比缺失更糟。
...
release manifest 自带 `schema_version`（1/2/3，`src/gpu_fault_release/regional_release_config.py`
的 `load_release_artifacts()`）：v2 起要求每个 component 的 `module_digest` 是小写 SHA-256；
v3 起还要求 `parse_delivery_identity()` 能解析出 delivery identity、锁定镜像与 node template
摘要，缺一即 `ReleaseError`。清单里 wheel/bundle 的相对路径按**清单所在**的 checkout 或源码
快照解析（`src/gpu_fault_release/__init__.py` 的 `containing_repository_root()`，以
`deploy/control-plane/regional` 与 `scripts` 两个目录为锚），而不是按运行中引擎自己的
`ROOT`：join-cluster 在操作员 checkout 内进程内跑引擎，若按引擎根解析，`VERIFIED` 会在
自己的树里找不到 `dist/<release-id>/`，把一次已成功的发布回滚掉。

锁定镜像的校验位置：`RegionalRelease.__init__` 经 `resolve_release_image()` 把 `GPU_FAULT_RUNTIME_IMAGE` 等环境变量
与 manifest 锁定镜像比对——同摘要不同引用采用环境引用；摘要不同只记入 `image_lock_conflicts` 并暂取锁定值。
`_dispatch_mode()` 在除 `IMAGE_LOCK_EXEMPT_MODES`（只发布 registry、不读任何镜像的 drain-cluster）之外的每个
模式开始前调用 `require_image_lock()`，一旦有冲突即 `ReleaseError` 并打印两个引用。于是
`GPU_FAULT_ADMIN_ALLOW_UNBOUND=1` 下用领先于部署的 checkout 跑 uninstall，其 drain 不会被镜像锁拦住，而会
应用或校验镜像的模式（deploy/upgrade/plan/verify/remove-cluster 等）仍然 fail closed。
...
`dcgm-diagnostic-zh-v2` 邮件，邮件字段完全来自结构化结果和上述规则表。
...
SMART 结论按驱动器缓存 300 秒，但 `smartctl --scan-open` 每个周期都重新枚举，因此
换盘或新挂盘不必等缓存过期。缓存只覆盖真正拿到结论的查询：`smartctl -H` 无法执行
或输出无法解析时，本轮发出 `smart_health_failed=1` 并带 `failure_mode=QUERY_FAILED`，
并且不开启缓存窗口，下一周期立即重试——否则一次解析失败会把"这块盘健康未知"冻结
300 秒。能正常返回但没有给出 PASSED/FAILED 结论的 smartctl 不算失败，可以缓存。
...
`watcher.py` 提供 runtime-neutral 状态机；Kubernetes 侧的实现按职责拆成 `completion_*.py`
十二个模块：`completion_controller.py` 只保留 `KubernetesCompletionController` 宿主类（reconcile
pass、`resume_tombstoned_attempt`）、`controller_from_environment` 与 console script 入口 `main`；
`completion_pod_parsing.py`（label/annotation 常量、Pod → Observation 解析）、
`completion_workload_stopper.py`（`KubernetesWorkloadStopper`，紧急 suspend）、
`completion_liveness.py`（进展预算、`note_progress`）、`completion_delivery.py`（attempt 投递、
failure containment、逾期紧急停止）、`completion_reconcile_loop.py`（list/watch 循环、
`run_watch_cycle` 去抖）；`completion_observation.py`（workload 对象读取与 `WorkloadVerdict`、
`MissingAttemptTracker`、`TerminalObservationCache`——终态观测区分"从 Pod 状态读到"与"因 Pod
缺失推断"两种来源，后者可被此后出现的 Pod 驱逐）；`completion_attempt_state.py`（`AttemptSpec`、
持久化 attempt 的恢复与驱逐、`publish_coverage_heartbeat`）与 `completion_attempt_store.py`
（`<name>-active` ConfigMap 里每个活跃 attempt 只存 spec 与 Pod 身份，不存日志快照/重启计数）；
`completion_outbox.py`（`KubernetesCompletionOutbox`：write-ahead outbox）与
`completion_outbox_records.py`（其 ConfigMap 文档的纯函数）；`completion_metrics_server.py`
（Watcher 自带的 `/metrics` + `/healthz`）。日志名统一借用 `gpu_fault.completion_controller`，
所以拆分前后的日志行逐字节相同。

write-ahead outbox 的语义：`/v1/attempts/failure-detected` 与 `/v1/attempts/terminal` 两条关键投递
先落到 ConfigMap `<name>` 的 `events.json`，再尝试投递；投递失败由 `replay` 重放，可重试失败持续
超过 `DEFAULT_MAX_RETRY_AGE_SECONDS`（24 小时）的记录进入 quarantine，容量不足时先驱逐 quarantine
记录腾出空间（`--replay-quarantined` 是一次性重放它们的操作员入口）；例行 attempt 状态放在第二个
对象 `<name>-active`，因为 ConfigMap 每对象 1 MiB 上限——两者同放一处时，N 个运行中 Pod 的例行状态
可把终态事件挤到写不进去。Watcher 的 `/metrics` 导出 `gpu_fault_completion_watcher_*`（reconcile
失败/次数、outbox 深度与 quarantine 深度、驱逐/过期计数、覆盖心跳、进展预算），`/healthz` 只在整个
投递预算（`progress_stall_budget_seconds`）内一次 pass 都没完成时才失败，空闲集群、慢 pass 与
API server 中断期间保持 green。
...
首次失败只产生一次 `FailureDetectedEvent`；Watcher 返回的
`FREEZE_EVIDENCE` / `STOP_DISTRIBUTED_ATTEMPT` 是意图，正常路径由控制面编译为
`FREEZE_EVIDENCE -> STOP_WORKLOADS` workflow，再经 Executor 停止整个 attempt。
关键 rank 全部结束，或失败后的 cleanup timeout 到期等终态条件满足时，生成唯一终态。

failure 事件持续未被控制面受理超过
...
```mermaid
flowchart TD
    A["观察 managed=true 的训练 Pod"] --> B["持续记录 attempt、rank、退出码<br/>node、GPU、workload allocation"]
    B -- "首次确认关键 rank 失败<br/>或 workload FAILED" --> F["产生一次 FailureDetected<br/>提交控制面"]
    F -- "控制面受理" --> C["幂等创建遏制 incident / workflow<br/>FREEZE_EVIDENCE → STOP_WORKLOADS"]
    F -. "持续未受理<br/>超过 fallback 超时" .-> E["Watcher 紧急 suspend<br/>保留日志和标注，继续补报"]
    C -. "Executor 异步停止整个 attempt" .-> B
    E -. "继续观测" .-> B
    B --> T{"满足终态条件？<br/>关键 rank 全部结束<br/>或失败后清理超时等"}
    T -- "否" --> B
    T -- "是" --> X["生成唯一 TerminalEvent<br/>FAILED / TIMED_OUT<br/>SUCCEEDED / STOPPED"]
    X --> Y["通过 outbox 投递和重试<br/>进入控制面终态决策"]
```

`CompletionWatcher.observe` 不把主动删除的非零退出或其他 incident 发起的终止
当作普通失败再次遏制。终态生成不等待遏制 workflow 完成，图中的虚线仅表示异步影响。
失败清理默认 120 秒；`controller_from_environment` 的紧急停止超时默认 30 秒。
投递在 outbox 中尚未确认受理，也会保持紧急停止计时。
代码依据为 `src/gpu_fault/watcher.py::CompletionWatcher.observe` 与
...
1. 已有同一 event_key 的 decision 时返回原决策并标记 `duplicate=true`，不重新定因。
   否则校验 profile；若 attempt 尚无被动遏制 incident、终态为 `FAILED/TIMED_OUT`、
   `workload_ids` 非空且显式 initiator（如有）不是其他 incident，则用终态字段幂等创建
   遏制 incident 与 `FREEZE_EVIDENCE -> STOP_WORKLOADS` workflow（与 failure-detected
   事件共用同一 event_key，先到者创建、后到者复用）。`STOPPED/SUCCEEDED` 不创建遏制，
   即便 rank 有非零退出码（规则 A / PREEMPT-033）。
2. 遏制补建使用 failure event_key 的独立幂等事务；随后在 terminal event_key 事务内
   再次检查 decision，保存 event、计划及其 incident/workflow、decision，避免重复投递
   或多副本竞争产生第二份恢复。旧版本遗留的“已有 event 但无 decision”可在重投时补决策。
3. 恢复 workflow 以 `predecessor_workflow_id` 指向遏制 workflow，由调度器保证先停后启；
   终态上报不再因遏制仍开放而返回 409，也不等待遏制执行完成。
4. 由其他 incident 发起的终止（`termination_initiator_incident_id` 指向非本 attempt
   被动遏制 incident），或显式 initiator 查不到 → `NO_ACTION`，不递归恢复。
5. 没有被动遏制时，`STOPPED` 或非失败终态 → `NO_ACTION` 并撤回该 job 的相关重启
   workflow。非失败按 `TerminalEvent.is_failure` 判断，也检查 rank 非零退出码。
   存在本 attempt 被动遏制 incident 的 `STOPPED`（无论是否带 initiator，
   DESTR-015 tombstone 竞争会丢掉
   该标注）继续恢复。
6. 在默认终态前后 10 分钟窗口内匹配可信、active、有建议动作、在 `ended_at` 未过期，
   且 node/GPU/fabric scope 与 allocation 相交的 marker。查询的是已入库记录，
   不是等待未来 10 分钟；决策持久化后，迟到 marker 不会触发本终态的重新决策。
   诊断类 marker（`COLLECT_EVIDENCE` / `RUN_DIAGNOSTICS` / `VALIDATE_NODE`，
   `gpu_fault.markers.marker_is_diagnostic`）若指向一条已存 incident，**不使该 incident 拥有
   作业恢复**——它只观察节点、不修节点，terminal 按其余可用 marker 决定；没有剩余
   marker 时才走无证据路径。被跳过的 marker 不在此处退役。
   没有存到 incident 的诊断 marker 仍按 `from_marker` 编出它要求的诊断计划。
   对其余 marker，若其指向另一 attempt 的 incident，且该 incident 的 workflow 已 `SUCCEEDED`，
   则在此处退役，不把上一次修复证据用于本次恢复。
7. 有 marker 时选择动作优先级最高者。
8. 所选 marker 的已存 incident 若关联的 workflow 包含 `RESTART_WORKLOAD`，
   返回 `NO_ACTION`，不创建第二次恢复；否则已存 incident 走 `after_incident`，只规划
   等该 incident `RECOVERED` 后的重启。incident 尚未存储时才走 `from_marker`。
9. 无 marker 且 allocation 存在时生成 `no-hardware-evidence:RESTART` 计划：一步 RESTART_WORKLOAD，重启次数由 restart_budget 在执行期限制；profile 无 WORKLOAD_RESTART 时退化为取证 + 升级。
10. allocation 缺失时生成 `COLLECT_EVIDENCE + ESCALATE_OPERATOR` 计划，节点列表为空，
    不猜测节点、不自动重启。此分支检查 allocation 是否为空，不等于校验
    `AllocationCompleteness.COMPLETE`。

```mermaid
flowchart TD
    A["收到 TerminalEvent"] --> B{"已有该 event_key 的决策？"}
    B -- "是" --> D["返回原决策<br/>标记 duplicate"]
    B -- "否" --> C["校验 Runtime Profile<br/>必要时补建被动遏制<br/>进入终态 event_key 事务并复查幂等"]
    C --> I{"其他 incident 发起终止<br/>或显式 initiator 无法确认？"}
    I -- "是" --> N["NO_ACTION<br/>不新建恢复"]
    I -- "否" --> U{"没有被动遏制，且<br/>STOPPED 或非失败？"}
    U -- "是" --> S["NO_ACTION<br/>撤回该 job 相关重启 workflow"]
    U -- "否" --> M["按 allocation 和时间窗口筛选 marker<br/>排除不应接管恢复的 marker"]
    M --> H{"命中有效 marker？"}
    H -- "是：选择动作优先级最高者" --> J{"marker 指向已存 incident？"}
    J -- "是" --> O{"原 workflow 包含重启步骤？"}
    O -- "是" --> N
    O -- "否" --> P["只规划 RESTART_WORKLOAD<br/>要求原 incident 达到 RECOVERED"]
    J -- "否" --> K["按 marker 动作生成计划<br/>重启、修复、诊断、隔离或升级"]
    H -- "否" --> L{"allocation 非空？"}
    L -- "否" --> Q["取证 + 升级人工<br/>禁止自动重启，不猜测节点"]
    L -- "是" --> R{"Profile 有可执行的重启能力？"}
    R -- "否" --> Q
    R -- "是" --> W["无硬件证据：单步重启计划<br/>受同一 job 的 restart budget 限制"]
    P --> V["编译为带 fencing 的恢复 workflow<br/>有遏制 workflow 时设置为前驱"]
    K --> V
    Q --> V
    W --> V
    V --> SAVE["与事件、决策一起提交事务"]
    N --> SAVE
    S --> SAVE
    SAVE -- "有新 workflow" --> G["Dispatcher 等待前驱释放<br/>执行权限、预算及安全门禁"]
    G --> Z["对应 GPU 集群 Executor 执行<br/>回写 workflow / plan 状态<br/>重启时创建新 attempt"]
```

图示以 `src/gpu_fault/service.py::CompletionService` 的决策顺序为准；事务提交前
不向 Executor 下发新计划。计划仍须由 Profile 解析可执行 owner，否则 fail closed。
总览见[概要设计](概要设计.md#passive-terminal-overview)，角色交互见
[详细设计 v2 的被动终态时序](../docs/详细设计-v2.md#passive-terminal-sequence)。

### 2.8 Planner 与 Passive Compiler

`PlanBuilder` 把 marker 转换为 RecoveryPlan，并从 Effective Runtime Profile 解析每一步 owner。典型硬件动作计划：
...
Planner 只表达动作与目标节点；`RESTART_WORKLOAD` 的执行参数
（cluster/job/attempt/GPU 数/预算、avoid_node_ids、`requires_incident_state`）
由 Passive Compiler 结合终态事件，以及 `RecoveryPlan.avoid_node_ids`、
`RecoveryPlan.restart_after_incident_id` 推导。`SimulatedRecoveryExecutor`
在模拟前同样从 plan 字段读取这些前提。

重启配额只在控制面预检 `reserve_restart_budgets` 预留；下发时由
`issue_restart_authorization` 读回预留并签发 `RestartAuthorization`，数据面
`_restart_guard` 比对授权、incident 前提、避让节点与 GPU 数。
预算耗尽由 `withhold_exhausted_restart` 扣留该 `RESTART_WORKLOAD`：
预写 FAILED 执行记录、把该步骤加入 `superseded_step_indexes` 并发送预算耗尽通知，
不调用它的 adapter；其余 cordon / stop / reset / validate / restore 等已规划步骤
继续按安全门禁执行，workflow 最后以 `FAILED` 收尾。缺少、冲突或非法重启上下文
才由 `fail_restart_preflight` 在任何 adapter 执行前使整条 workflow 失败。
代码依据为 `src/gpu_fault/execution/restart_budget_preflight.py`。

前驱依赖只保证执行顺序，不证明前驱成功。每个计划最多一个 incident 前提：
本 attempt 被动遏制存在时，无证据重启与没有其他前提的 marker 计划以该 incident
为前提（遏制 STOP 失败使 incident `ESCALATED`，守卫拒绝重启）；`after_incident`
计划仍只以其节点 incident 为前提，遏制结果不额外门控。
`_incident_premise` 在执行期要求 `RECOVERED`；尚未恢复则等待，`QUARANTINED`、
`ESCALATED` 或无法核实状态则拒绝重启。
...
普通 fatal access/trunk SXID、code-specific full-reset SXID 与同一 attempt 上的 XID
使用相同的 attempt 聚合键：`cluster_id + job_id + attempt_id`
（`IncidentOrchestrator._attempt_group_key`），`official_action` 不参与分组，因此
access 与 trunk 事件、SXID 与 XID 事件会进入同一个 workflow。第一条事件按其节点编译
SXID 计划；之后每条事件先为自己的节点编译同一份单节点计划，再交给
`DispositionApplier` 套用合并裁决，与 XID 家族共用同一实现：每个新节点——无论
reset 类别与首节点相同（两条 trunk、两条 access）还是不同（access 的 `RESET_GPU`、
trunk 的 `RESET_ALL_GPUS_NVSWITCHES`、XID 79 的 `RESTART_NODE`），无论到达时 workflow
是否已开始执行——都作为 `branch:<node>` DAG 分支挂在 `branch:initial` 旁边，分支内的
reset step 只命名本节点，并只携带本节点的 GPU、SXID 与 fabric partition 映射；任何
SXID 合并都不会产出 `node_ids` 跨节点的 reset step（区域 executor 没有 barrier
coordinator，`CommandDispatch._barrier_hold` 会把这样的 step fail-closed 持有）。同一
节点的更高 rank 事件在计划未开始时原地替换计划，已开始后排为该节点分支的
successor；同一节点、同一类别的追加 GPU 只拓宽该节点 step 的 GPU 范围。无论几个
节点、几种 reset 类别，整个 workflow 只有一个 `shared` `STOP_WORKLOADS` 和一个
`join` `RESTART_WORKLOAD`：join 门禁在全部分支完成 reset/validation 后才放行，整个
训练任务只重启一次。incident 的 `official_action` 取 recovery rank 更高的动作，后到
的低 rank 事件不会将其降级。不同 attempt 不会混合。
...
kmsg 的**读取与投递是两段**：读线程把记录压进一个进程内有界队列（2048 条），一个
投递线程排空它。读线程永远不等投递——`HttpEventSink` 每条记录最多花约 47 秒重试
才交给 outbox，而内核 ring 在这段时间里一直在覆盖旧记录——所以背压只能落在队列上：
队列满时丢**最老**的一条，保留最新的。收到 SIGTERM 时队列不再丢弃，而是在停机预算
内同步写进 outbox，写不下的才算丢失。投递线程自己死掉会被计数并替换，替换超过 3 次
后不再替换，改为读线程内联投递。

投递侧的损失全部进 kernel collector 的周期健康摘要，以 `<名字>:<计数>` 追加到
`edge_filter_reasons`，计数为 0 时不出现：

| 计数器 | 含义 |
| --- | --- |
| `delivery_failures` | 投递失败次数 |
| `kmsg_overflow` | 内核 ring 覆盖导致的读取丢失 |
| `boot_time_reestimates` | monotonic→wall clock 的 boot 时间重估次数 |
| `delivery_queue_drops` | 队列满时丢掉的 kmsg 记录——**这一项非零就是真的丢了 XID/SXID 证据** |
| `health_summary_queue_drops` | 队列满时丢掉的健康摘要；丢的是存活报告，不是故障证据，因此与上一项分开计数 |
| `delivery_buffered_at_shutdown` | 停机时改由 outbox 承接、后续重放的记录数 |
| `delivery_dropped_at_shutdown` | 停机预算内既投不出也写不进 outbox 的记录数，已丢失 |
| `delivery_thread_deaths` | 投递线程自身死亡并被替换的次数 |
...
判据是作用域，不是"时间上挨得近"。匹配上的诊断类 marker（见上 §终态处理第 4 步）不会让
其 incident 成为作业恢复的拥有者：一条 WARNING `RUN_DIAGNOSTICS` marker 曾把作业的重启门
挂在一个从未修过任何东西的诊断 incident 上，直到 marker TTL 到期一小时。
...
5. 数量一致或精确批准后，比对控制面签发的 `RestartAuthorization`（reservation_id、job、attempt、GPU 数、预算）；restart budget 本身已在 claim 预检 `reserve_restart_budgets` 中原子预留一次，Adapter 不再写预算。
6. 预算耗尽由控制面预检判定：workflow 直接 `FAILED`，不调用 Adapter、不创建 workload，预算耗尽通知由预检发出。Adapter 没有拿到授权、或授权与眼前 workload 不符时 fail-closed，并在 details 标记 `restart_submitted=false`，供终态写入释放预留。
...
11. `STOP_WORKLOADS` 是幂等 containment：若目标 Job/PyTorchJob/JobSet 已被外部
    ...
    因而源对象缺失时继续 fail-closed。Adapter在比对`RestartAuthorization`前再次读取源对象，
...
历史的同步路由 `POST /v1/node-actions` 已经删除，提交与取结果被拆成两个动作：
`submit` 校验通过后立即返回 `NodeActionSubmission`，动作在 Agent 内部的动作池里
执行；结果由 `result` 查询，查询本身也要带同一 HMAC secret 的
`issued_at`+`signature`。重启类动作只能走这条路径——同步路由在节点重启时必然丢
响应，而它的存在会让调用方以为"没拿到响应"等于"动作没执行"。
...
7. 本地 ledger 保证 exactly-once：同一 `command_id` 重放拿到同一条回执，
   而 `command_id` 被复用于**另一个请求体**时回 409 `COMMAND_ID_REUSED`
   （`retryable=false`、`requires_new_command=false`）：这是 workflow 缺陷而不是硬件
   故障，步骤终态失败并交操作员，没有任何一层会自动铸新的 `command_id`。
8. reset 前调用 `nvidia-smi` 并检查 `/dev/nvidiaN` 持有者。

Node Agent 不主动停止 HMA、Fabric Manager、device plugin 或训练任务。

`COLLECTOR_OUTBOX_MAINTENANCE` 是 node-action 的第 16 个 operation，只做 metadata-only 的维护：
管理员命令 `gpu-fault-admin collector-outbox` 把节点本地 `gpu-fault-collector outbox` 的检视/requeue
作为 NODE_ACTION 步骤下发到节点，只回传元数据，不搬运载荷；`requeue-dead` 需要 `--yes`，远端不提供
`--force`。
...
Aurora 中的 AgentRecord 与安装资源注册表只作为运行期事实源，不能作为 Aurora 自身销毁流程
的进度存储。`uninstall`（`src/gpu_fault/admin/uninstall.py`）的进度全部写在 state-dir 的外部
文件里：`uninstall/state.json` 记录 phase
`STARTED → REGISTRY_EXPORTED → KUBERNETES_VERIFIED → NON_AURORA_DELETE_IN_PROGRESS →
READY_TO_DELETE_AURORA → COMPLETED`；停止 writer 前先把注册表快照导出为带 `.sha256` sidecar
的 `installation-resources-before.json`，随后依次落 `installation-resources-delete-plan.json`、
`installation-resources-pre-aurora-delete.json` 与 `installation-resources-final.json`；
Kubernetes 侧清理单独写 `kubernetes-cleanup.json`，其 phase 必须到达 `CLEANUP_COMPLETED`
才进入 `KUBERNETES_VERIFIED`，上一轮没到该 phase 的清理记录会被移到一旁而不是当成已完成复用。
Aurora 删除线程（`AuroraDeletion`）在 `KUBERNETES_VERIFIED` 之后立即发出（关删除保护、reader、
writer、cluster），与非 Aurora 资源的分优先级删除并行；`READY_TO_DELETE_AURORA` 记录非 Aurora
阶段结束，随后等待 Aurora 删除完成并核对保留的最终快照。最终状态由
`installation-resources-final.json` 中每条资源的 `DELETED/DETACHED/PRESERVED` 收口，任何非终态
资源都使 uninstall 失败。
...
执行器把一次可重试失败（DNS 抖动、K8s 409/5xx）上报为 WAITING 时，会在原有
`result_details` 之上叠加错误键而不是替换：`observed_isolation` 等适配器跨轮次
的记忆必须保住，否则一次抖动就让上面的复核失去依据；回放给适配器时再把这几个
错误键剥掉（`cluster_executor.adapter_facing_details`）。
...
多 worker Pod 暴露的进程内 counter 要么携带稳定 `process_id` label，要么像
`gpu_fault_store_io_rejections_total` 那样经共享内存跨进程聚合后按 `reason`
（`capacity`/`deadline`/`backend_unavailable`）导出；否则同一 Pod 的 scrape 随机命中不同
uvicorn 进程时会把 0/非零值误拼成同一 counter，产生持续的虚假 rejection rate。
...
**编译期 BLOCKED 的无操作 workflow 与终态 workflow 遗留的 open command，不再需要管理员命令。**
还有一种记录连上面两层都碰不到：Runtime Profile 缺少某项能力的执行 owner 时，`compile_steps`
在任何步骤执行前就把 workflow 标为 `BLOCKED`（`blocked_reasons` 形如 `no executable owner for
efaDriverRemediation`）。它没有租约、没有 step execution、没有 remote command、没有 source
plan，containment 前置动作也没跑，因此节点上没有它的隔离；但它含破坏性操作，`workflow_safety`
preflight 与 Profile 切换 finalize 门都把它计为活跃，恰好挡住会补上该 owner 的发布。与之相邻的
另一种形态是终态 workflow（`FAILED`/`SUCCEEDED`/`BLOCKED`/`SUPERSEDED`）遗留的 open remote
command：旧版执行器在步骤触及等待上限时只判步骤失败、不取消该步的 remote command
（`execution/step_bounds.py` 现已先取消再失败），一条 `FAILED` 的 `CHECK_MECHANICALS` 可把
command 永远留在 `WAITING`，而升级门要求 open command 为 0。

这两种形态过去各有一个 `workflow-reconcile --mode ...`；每次实际使用都是为了解开发布 preflight，
且它们检查的每个条件都是 Store 谓词，所以现在由 dispatcher 的周期清扫
`WorkflowDispatcher.sweep_stuck_records`（`src/gpu_fault/execution/dispatcher.py`）自动关闭，判定
**按谓词不按年龄**，每个 tick 从新读取的记录重新推导：`gpu_fault.compile_blocked` 以
compare-and-set 的 `save_workflow` 把编译期 BLOCKED 记录写成 `SUPERSEDED`，`preemption_reason`
携带原 `blocked_reasons`，同一写入追加 actor 为 `dispatcher` 的 `OPERATOR_RECONCILED` 事件，不写
incident；`gpu_fault.orphaned_commands` 只对终态 workflow 的 open command 调用 Store 的
`cancel_remote_commands_for_workflow`（`PENDING`/`WAITING` 立即 `FAILED`，`LEASED` 登记取消请求由
执行侧履行），活 workflow 的 command 一律不碰。同一清扫还包含 `_supersede_abandoned_generations`
与 `_revoke_retired_generations`。发布 preflight 的探针用同一份内联谓词把这些形态排除在活跃计数
之外（`tests/regional/test_release_workflow_safety.py` 钉住两份拷贝）。

`gpu-fault-admin workflow-reconcile` 因此只保留一种需要节点证据的形态：BLOCKED 且从未改动节点、
或其 incident 已被**后续** workflow 恢复且节点已由 `RESTORE_SCHEDULING` 放回的记录——证明需要
控制面没有的节点证据（无 cordon、无 quarantine taint、无 gpu-fault 隔离注解），由
`src/gpu_fault/admin/workflow_reconcile.py` 经站点的 GPU kubeconfig 读取后交给 CPU ingress Pod 内
的 `gpu_fault.workflow_reconcile` 执行。一次调用同时 plan+apply：计划在 Pod 内构建，apply 前立即
重建并逐字段比对（`_plan_drift`），apply 携带运行时摘要、admin 摘要与操作员 STS 身份写入
`OPERATOR_RECONCILED` 事件；`--dry-run` 只打印计划，Pod 与磁盘都不写；否则计划与结果归档到
`workflow-reconcile/history/<digest>/`，`records_deleted` 恒为 0。它没有 `--mode`；其余选项
`--close-incident/--close-escalated/--close-quarantined` 关闭 ESCALATED/QUARANTINED incident，
后者要求节点隔离已消失并剥离手工放行 taint 遗留的注解。操作步骤见《管理员日常运维》§10.1
（自动关闭）与 §10.2（`workflow-reconcile`）。
...
在 warm spare 路径必须继续使用（`hyperpod_spares.py` 的
`HyperPodSpareCoordinator._resolve_targets()` 以 alias/retired alias 解析故障节点身份）。

生产 profile 的 owner 是静态的：区域站点的 profile 源自模板
`config/runtime-profile.regional-hyperpod-safe.example.yaml`（`workloadRestart` 为
`mode: OWN`、owner `gpu-fault-kubernetes-adapter`），经站点 release 配置的
`runtime_profile.source` 由 `src/gpu_fault_release/regional_runtime_profile.py` 的
`render_runtime_profile_payload()` 渲染注册；旧单集群脚本 `deploy/hyperpod/deploy.sh` 同样把
`workloadRestart` 固定为该 owner 并无条件 `"mode": "OWN"`，但它已不是区域架构的运行时路径，
只剩契约测试还读它。在新约束下这个静态值
...
手工 CLI `hyperpod_cli.py` 仍调用 `resolve_recovery_ownership()`
...
（`HyperPodManagedRecoveryObserver.observe()` 内是唯一调用点），没有 workflow step 就都不发生。
后果：旧 agent 停在 `ACTIVE` 不被 fence，新节点不经本方案校验直接进调度池；
且若新实例上的 agent 沿用同一 `node_id` 注册，`fleet.py` 注册路径 `_next_agent_record()` 的
...
`token_rotation_expires_at`、`enabled`（默认 `True`）、`lifecycle_state`
（`RegionalClusterLifecycle`：`PENDING/ACTIVE/FAILED/ROLLED_BACK/DRAINING/REVOKED`，默认
`ACTIVE`）、`synthetic`/`synthetic_run_id`/`synthetic_expires_at`、`allowed_namespaces`、
`agent_endpoint_allowed_cidrs`（校验为合法网络，排序去重）、`created_at`、`updated_at`。

`lifecycle_state` 是 join/remove 在线过渡的状态机，由 §2.15.1 的 transition 路由推进；每个
cluster-token 请求先过 `regional_cluster_request_allowed()`：`ACTIVE` 全放行；`REVOKED`/
`ROLLED_BACK` 全拒绝；其余状态放行 `GET/HEAD` 与三条 bootstrap 路径（`/v1/fleet/agents/heartbeat`、
`/v1/fleet/readiness`、`/v1/regional/executors/readiness`），`PENDING` 额外放行
`/v1/collector-events/*`（join 在激活前要用这些事件证明采集就绪，claim 仍被拒，所以放进来的
遥测不可能启动节点动作），`FAILED` 拒绝其余，`DRAINING` 只再放行 `hyperpod-submissions/outcome`
与 `{command_id}/renew`、`/result`，让在途命令能收口。`synthetic_*` 供验收 fixture 注册带 TTL
的合成集群，过期的合成注册在加载时被跳过（`configured_regional_registrations()`）。
...
控制面只持久化摘要，不持久化 token 明文。`GPU_FAULT_REGIONAL_CLUSTERS_JSON` 只是首次
bootstrap 的**种子**：`sync_regional_cluster_registry()` 在 Store 里还没有 durable head 时才把它
写成第一代 revision；一旦 head 存在，head 就是事实源，Secret 与 head 不一致只作为
`secret_drift` 上报（§2.15.1）。该变量缺失时区域模式仍然启动失败，且区域模式不接受单值
...
注册表在 `gpu_fault_objects` 里是三种 kind（模型在 `regional.py`）：`regional_registry_head`
（`RegionalRegistryHead`：`generation` + `content_sha256`）、`regional_registry_revision`
（`RegionalRegistryRevision`：某一代的完整 `registrations`、`previous_generation`、发布时的
`required_member_ids`、`reason`）和 `regional_registry_member`（`RegionalRegistryMember`：每个
控制面进程一行，`member_id`、`service_role`、`release_id`、当前服务的 `generation`/
`content_sha256`、`ready`、`error`、`last_seen_at`）。head 的摘要必须等于其指向 revision 的摘要，
否则 `durable regional registry head digest mismatch` 直接拒绝启动。

每个角色（ingress/worker/spool-worker）都跑一条 `gpu-fault-regional-registry` 线程
（`RegionalRegistryRuntime`，`src/gpu_fault/regional_registry_runtime.py`）：每
`GPU_FAULT_REGISTRY_POLL_SECONDS`（默认 1）读一次 head，代数或摘要变化时加载对应 revision 并整体
替换进程内快照；随后写自己的 member 心跳行——内容（generation/摘要/ready/error）变化时立即写，
否则每 `stale/3` 秒重写一次（生产清单 `GPU_FAULT_REGISTRY_STALE_SECONDS=90`，应用默认 10；90 秒
窗口下全站约 1.2 次/秒）。`is_ready()` 要求快照对准 head 且最近一次成功刷新在 stale 窗口内，不
ready 时 cluster-token 请求得到 503 + `Retry-After`；一次瞬时的连接关闭不再算作不 ready。启动时以
`GPU_FAULT_STARTUP_STORE_RETRY_SECONDS`（默认 120）为预算等待 Store，避免 Aurora writer failover
期间新 Pod 陷入需要 Aurora 才能起来的重启循环。心跳行超过
`GPU_FAULT_REGISTRY_MEMBER_RETENTION_SECONDS`（默认 86400）未更新即被周期清理删除。

在线变更走四条 `execution-token` 路由（`src/gpu_fault/app/routes/regional_registry.py`）：
`GET /v1/regional/registry/status` 返回 `RegionalRegistryStatus`（当前代与摘要、`cluster_states`、
required/acked/missing/active 成员、`members` 与 `converged`）；`POST /v1/regional/registry/revisions`
以 `expected_generation` 做 CAS 发布下一代（不匹配 409）；`POST /v1/regional/registry/rollback` 把
`target_generation` 的 registrations 作为新一代重新发布，而不是回写旧代；
`POST /v1/regional/registry/clusters/{cluster_id}/transition` 只改一个集群的 `lifecycle_state`，
最多重试 8 次 CAS。每次发布把当时活跃（stale 窗口内有心跳）的成员集合记为 `required_member_ids`，
发布后立即 `refresh_once`。收敛规则 `registry_revision_converged()`：`required_member_ids` 中每个
成员都已 ready 且服务该代——但已离场的成员（心跳行缺失或超过 stale 窗口）不再阻塞：它是发布时
快照里正在滚动退出的旧 Pod，按注册表自己的活性规则已不服务任何流量，等它 ack 会让发布永远等不到；
空的 required 集合在第一次轮询就收敛。join/remove 在这条路径上推进 `PENDING → ACTIVE` 与
`DRAINING → REVOKED`，`rotate-token` 经同一发布路径写入轮换重叠窗口。
...
另有三组字段：`batched_steps: list[BatchedStep]`（复合命令，见下文"协议版本与进度回报"；每项带
自己的 `step_index`、`step` 与 `idempotency_key`，命令自身的 `step/step_index/idempotency_key` 仍是
这串步骤的第一个，`covered_step_indexes` 给出全部覆盖的步骤；为空时序列化整体省略该字段，使协议
1/2 的 executor 用 `extra="forbid"` 解码 claim 响应不会 422）；`cancellation_requested_at`/
`cancellation_reason`（控制面对 `LEASED` 命令只能登记取消请求，由执行侧在步骤之间经 lease guard
履行；`PENDING/WAITING` 的取消直接置 `FAILED`）；`result_details`/`error`/`status_source`（回执
写回，`result_details` 在下一次 claim 时随命令回放）。
...
（1–25，默认 1）、`lease_seconds`（10–7200，默认 60）、三个兼容性字段
`executor_protocol_version`（默认取 legacy 值 1）、`executor_artifact_sha256`、
`executor_compatibility_digest`（后两者可选，64 位小写十六进制），以及 long-poll 的
`wait_seconds`（0–30，默认 0）。`wait_seconds>0` 时控制面在该集群没有 `PENDING` 命令时持有空
claim，等 Store 发布 `REMOTE_COMMAND` 唤醒后再 claim 一次（`src/gpu_fault/app/remote_command_wakeups.py`，
每个 uvicorn 进程一条 LISTEN 连接，首个订阅时才建立，worker/spool 角色从不打开）；实际等待取
请求值、服务端上限（`RemoteCommandWakeupHub.max_wait_seconds`，默认 25 秒）与请求预算剩余量
（`bounded_claim_wait_seconds()`：`REQUEST_DEADLINE` 扣 3 秒保留）三者的最小值，监听连接不在时
退化为 2 秒的旧轮询间隔。它是可加的可选请求字段，不算协议版本提升。`RemoteCommandClaim` 返回
`commands` 列表。
...
**协议版本与进度回报。** `src/gpu_fault/regional_compatibility.py` 定义
`CURRENT_REGIONAL_EXECUTOR_PROTOCOL_VERSION = 3`、`REMOTE_STEP_BATCHING_PROTOCOL_VERSION = 3`、
legacy 1；Agent 侧 `src/gpu_fault/fleet_compatibility.py` 的 `CURRENT_AGENT_PROTOCOL_VERSION = 3`。
executor 协议 3 的内容是复合命令：`src/gpu_fault/remote_step_batching.py` 在派发一个 node-action
步骤时，把其后同一节点、同一 `execution_owner`、无分支且无显式依赖的连续 `BATCHABLE_OPERATIONS`
（`QUIESCE_GPU_SERVICES`、`VERIFY_NO_GPU_CLIENTS`、`RESET_GPU`、`RESTORE_GPU_SERVICES` 等单节点
操作；多节点 barrier reset、长诊断/修复与其他 adapter 的操作除外）装进同一条命令的
`batched_steps`，省掉每步一次控制面 ↔ executor 往返。`RemoteStepBatchingPolicy.active` 要求
`GPU_FAULT_REMOTE_STEP_BATCHING`（默认 true）打开**且**兼容策略接受的最低 executor 版本 ≥ 3——
claim 响应多了一个旧 executor 会拒收的字段，所以这是版本提升而不是可加字段；控制面只在全部已接受
的 executor 版本都 ≥ 3 时铸造复合命令。executor 每完成一步就
`POST /v1/regional/executors/{command_id}/progress`，载荷
`RemoteCommandProgress(executor_id, lease_token, batched_results)`，`batched_results` 以步骤序号为键、
值为 `BatchedStepResult(status, status_source, details, error)`；它与续租同受 lease 围栏，控制面
收到后立即唤醒 dispatcher 让下一覆盖步骤推进，终态 `/result` 仍携带完整 `batched_results`，进度贴
丢失（旧控制面答 404）只多付延迟，不影响正确性。
...
### 2.17 Cluster Action Executor（包 `gpu_fault.cluster_executor`）

原单文件 `cluster_executor.py` 已按层拆成包（`__init__` 原样再导出 36 个名字，
`gpu_fault.cluster_executor:main` / `:readiness_probe` 两个 console script 不变），当前七个模块：
...
`lease.py`（`CommandLeaseWatch`、`CommandLifecycle`：续租线程、结果回报重试、放弃持租）、
`dispatch.py`（`CommandDispatch`：本地校验 `_validate`、fleet preflight、barrier 持有、失败分类、
`execute`）、`executor.py`（`ClusterActionExecutor` claim 循环、计数器、SIGTERM、
`SpareReservationSweep`）、`bootstrap.py`（`executor_from_environment`、`readiness_probe`、`main`）、
`batching.py`（`execute_batched_command`：复合命令按序执行，每步重建自己的 `WorkflowStepContext`，
步骤之间查询 lease guard，收到取消或不再信任租约即停止并回报已完成部分，
`STOPPED_BETWEEN_STEPS_STATUS_SOURCE`）与 `metrics.py`（`ClusterExecutorMetrics`/
`cluster_executor_metrics`：`gpu_fault_cluster_executor_*` 系列，与 readiness breadcrumb 读到的是
同一组计数；`start_executor_metrics` 在 `GPU_FAULT_CLUSTER_EXECUTOR_METRICS_PORT`（默认 9111）
暴露 `/metrics` 与 `/healthz`，容器 liveness 探针读后者）。
所有层的 logger 名仍是 `gpu_fault.cluster_executor`；唯一可见变化是异常类的限定名变为
...
`GPU_FAULT_CLUSTER_EXECUTOR_CLAIM_WAIT_SECONDS`（默认 20：claim long-poll，0 退回纯轮询且不发送
该字段，见 §2.16）、`GPU_FAULT_CLUSTER_EXECUTOR_LEASE_SECONDS`（默认 120）、
`GPU_FAULT_CLUSTER_EXECUTOR_BATCH_SIZE`（默认 5）、
`GPU_FAULT_CLUSTER_EXECUTOR_MAX_CONCURRENT_COMMANDS`（默认 5）、
`GPU_FAULT_CLUSTER_EXECUTOR_LEASE_FAILURE_LIMIT`（默认 3）、
`GPU_FAULT_CLUSTER_EXECUTOR_TRANSPORT_DEGRADED_BACKOFF_AFTER`（默认 3）、
`GPU_FAULT_CLUSTER_EXECUTOR_MAX_EXECUTION_SECONDS`、
`GPU_FAULT_CLUSTER_EXECUTOR_METRICS_PORT`（默认 9111）、
`GPU_FAULT_CLUSTER_EXECUTOR_READINESS_TIMEOUT_SECONDS`（默认 8）、
`GPU_FAULT_CLUSTER_EXECUTOR_CLAIM_STATE_PATH`（默认 `/tmp/executor-claim-state.json`）、
`GPU_FAULT_CLUSTER_EXECUTOR_ID`（默认 `<cluster_id>/<hostname>`）、
...
| --- | --- | --- | --- | --- | --- |
| `ingress` | `gpu-fault-api-ha` | 3 | 4 | 8080 | 只有四条公共项（见 §2.18.2） |
| `worker` | `gpu-fault-control-worker` | 6 | 4 | 8081 | 全部（见 §2.18.2） |
| `spool-worker` | `gpu-fault-telemetry-spool-worker` | 0 | 1 | 8082 | 四条公共项 + spool 重放两条 |
...
后者还要求异步投递且 dispatcher 开关为真。

每个角色都无条件启动四条公共项（`src/gpu_fault/app/lifespan.py`）：线程
`gpu-fault-collector-metrics-snapshot`；asyncio 任务 `gpu-fault-event-loop-lag`（采样事件循环
滞后，导出 `gpu_fault_event_loop_lag_seconds`）；线程 `gpu-fault-regional-registry`（§2.15.1 的
注册表运行时，区域模式下三个角色都有）；线程 `gpu-fault-process-metrics`（每 5 秒把本进程完整的
`/metrics` 渲染发布到 `/dev/shm/gpu-fault-process-metrics/<pid>.json`，供同 Pod 其他 uvicorn 进程
合并，见 §7；`POD_UID` 缺失时线程立即返回）。

#### 2.18.3 通道注册表

`src/gpu_fault/channel_registry.py` 是所有采集入口的单一真源：10 条通道，每条声明
...
`/v1/attempts/coverage` 是 Completion Watcher 的 `WorkloadCoverageHeartbeat`：每完成一次没有看到
任何运行中 Pod 的完整 pass 发一条（每个 watcher 间隔至多一条），载荷不含 attempt/node，排序键是
cluster，`latest_wins` 让每个集群只留一条 pending 行——旧心跳输给新心跳是预期结果，不是丢写。
十条通道都带 `receipt`（入队后可经 `GET /v1/processor/requests/{request_id}` 查回执）；
`incident_scoped` 标记载荷可能进入 `incident.node_ids` 的通道（collector-health 之外的六条
collector 通道），processor 必须给它们节点关联键，聚合门才能看到在飞的同节点载荷。
...
| 单集群队列深度 | `GPU_FAULT_PROCESSOR_MAX_CLUSTER_QUEUE_DEPTH` | 4096 | 1000 |
| 故障预留（全局） | `GPU_FAULT_PROCESSOR_FAULT_RESERVED_QUEUE_DEPTH` | 8192 | 深度的 2/5 |
| 故障预留（单集群） | `GPU_FAULT_PROCESSOR_FAULT_RESERVED_CLUSTER_DEPTH` | 512 | 深度的 2/5 |
...
低 CPU/GPU 利用率只在控制面能为该节点解析出唯一一个真实受管 attempt（attempt
observation 中处于 PENDING/RUNNING、且在本节点仍有未终止容器）时才计时；collector
自报的 `ACTIVE` workload 状态只是环境变量派生的预过滤，不足以启动计时，解析不到或
解析出多个 attempt 都按空闲样本处理并重置持续窗口，避免占位任务持有的空闲节点误报。
finding 会带上解析到的 `job_id`/`attempt_id`。默认规则如下：
...
持续类信号按 GPU 设备逐个跟踪（一个节点上将来可能跑多个任务），但 incident 按节点
收口：同一 cluster + 节点上若已存在由同一 `metric_name` + `signal` 打开、且仍处于
`ACTION_PENDING`（诊断排队或执行中）或 `ESCALATED`（等待管理员关闭）的 incident，
后到的同信号 finding（无论哪张 GPU）只把事件链接到该 incident、追加一条有界 reason
并刷新 `updated_at`，不再新建 incident、workflow，也不再发邮件；
计入 `gpu_fault_workflow_merge_record_only_total{reason="unsettled_host_resource_incident"}`。
不同 metric、不同节点仍各开各的；incident 一旦 `RECOVERED`（管理员已关闭）或
`QUARANTINED`，下一条 finding 照常新开。
...
版本或二进制不匹配会使节点安装失败。节点运行时 venv 的依赖闭包同样只来自
`requirements/node-runtime.lock`（`pip install --require-hashes --no-deps`），该 lock
是 `collectors` extra 的完整导出，必须覆盖 node-runtime wheel 闭包里任意深度的每个
第三方导入：`prometheus-client` 只在 DCGM 采集器函数内部导入，2026-09-09 一份把它
剔除的 lock 装到金丝雀节点后采集器全部 ModuleNotFoundError、节点零指标、安装校验
失败；`test_node_runtime_lock_pins_every_third_party_import_of_the_node_closure`
按闭包逐个导入核对 lock，防止再次发生。
...
`nvidia-smi` 整体失败——一个 UUID 都没列出，而 `/proc/driver/nvidia/gpus/` 下仍有已
绑定的 GPU 目录——不判 mismatch。驱动升级后未重启会让整个机队同时出现
"Failed to initialize NVML"，把它当成掉卡就等于机队级 `REBOOT_NODE`。这一轮只发
`gpu_inventory_expected_count` 和 `gpu_inventory_discovered_count`（后者取 `/proc`
下的已绑定 GPU 目录数），两者都带 `failure_mode=DRIVER_QUERY_FAILED`，既不发
`gpu_inventory_mismatch` 也不发 `gpu_inventory_active_count`，连续不匹配计数保持不动。
代价是 latest-metrics 里的 `gpu_inventory_active_count` 会停在 NVML 失败之前的旧值：
判读 NVML 故障要看 `collection_errors` 和 `DRIVER_QUERY_FAILED` 样本，不能看 ACTIVE
数。只有部分 GPU 不答（仍列出了一部分 UUID）时才照常产出 inventory 样本，并额外带上
同一个 `failure_mode=DRIVER_QUERY_FAILED` 标签。
...
### 2.23 管理员 CLI 与发布内核 `src/gpu_fault/admin/`

`gpu-fault-admin` 是站点唯一的变更入口（`status`、`deploy`、`config`、`workflow-reconcile`、
`failure-domain-map`、`rotate-token`、`submit-remediation`、`collector-outbox`、`join-cluster`、
`remove-cluster`、`uninstall`）。它不参与故障判定，但与控制面共享 Store 模型、鉴权边界（§4 的
execution-token 路由）与 §2.22 的安装资源注册表。构建、签名 bundle、Profile 审批与发布编排的完整
契约在《开发者部署实现》§8–§9，本节只记录运行形态里最容易看错的四件事。

**deploy 是两跳。** 外层 `gpu-fault-admin deploy`（`src/gpu_fault/admin/deploy_command.py` 的
`run_public_deploy`）经 `src/gpu_fault/admin/source_deploy.py` 的 `run_source_deploy` 以当前解释器
执行 `scripts/staging_deploy.py`：校验源码树不携带线上身份、跑源码影响门禁、准备源码快照、构建/
验签 deploy-host bundle，再用 `scripts/setup-deploy-host.sh` 把 bundle 装进
`<state-dir>/deployer-venv`，并在该 venv 根写下绑定文件 `gpu-fault-managed-state-dir.json`
（`{"schema_version": 1, "state_dir": ...}`）。随后以 `<deployer-venv>/bin/gpu-fault-admin preflight
--state-dir` 与 `... deploy --prepared-source-release --repo-root <快照>` 重入内层：
`run_prepared_deploy` 在 `administrator_operation_lock` 内做 bootstrap，接着自动发布，最后 join
`pending_gpu_cluster_arns` 里待加入的 GPU 集群。外层全程持有 `site_operation_lock`，分类
（`UNCHANGED`/`DEPLOY_HOST_ONLY`/`APPLICATION_RELEASE`）与应用之间不再重读站点。已安装的
deploy-host 只认绑定的 state-dir：CLI 入口在执行任何命令前调用 `enforce_deploy_host_state_dir`，
对任何其他 `--state-dir` 直接拒绝，从源码 checkout 运行（没有绑定文件）则不受限。发布制品路径按
清单所在快照解析（§2.2）。

**bootstrap 的 `initial_deploy_target` checkpoint。** 首次部署在 `site.yaml` 存在之前就把目标
CPU/GPU 集群记进 `bootstrap-state.json`（`src/gpu_fault/admin/bootstrap_site.py` 的
`bind_initial_deploy_target`）：resume 必须对准同一目标，否则
`initial deploy target differs from the persisted checkpoint`。一旦 site 文档覆盖了全部目标——或
`remove-cluster` 已在 `removed_clusters` 下记录卸下了未到达的集群——checkpoint 判 `COMPLETE`，此后
由 site 文档而不是 checkpoint 说明站点管理哪些集群；它不再停在 `PENDING` 等一次恰好请求原始集合的
升级，所以 remove 之后的 CPU-only 升级和首次部署之后立刻 `deploy --gpu-cluster-arn NEW` 都不再被拒。

**join/remove/uninstall 各有一台状态机，进度只写外部文件。** `join-cluster`
（`src/gpu_fault/admin/cluster_join_state.py`）在 `<state-dir>/join-cluster/<ARN 摘要前 12 位>/state.json`
记 `completed_steps`（`PRECHECKED → DISCOVERED → LOCAL_INPUTS_READY → PREREQUISITES_READY →
CANDIDATE_READY → RELEASE_STARTED → JOINED → COLLECTORS_READY → VERIFIED`，phase `COMPLETED`；
已纳管时直接 `ALREADY_MANAGED`）与每步证据，失败时写 `failure{error, after_step, recorded_at}`，
phase 进入 `ROLLED_BACK`/`ROLLBACK_FAILED`；进行中的 attempt 若发现 site 非成员字段漂移即拒绝续跑。
`remove-cluster`（`src/gpu_fault/admin/cluster_removal.py`）在
`<state-dir>/remove-cluster/<cluster_id>/state.json` 按 `DISCOVERED → CONTROL_REGISTRY_DRAINING →
KUBERNETES_QUIESCED → KUBERNETES_REMOVED → CONTROL_REGISTRY_REMOVED → AWS_DETACHED →
AURORA_UPDATED → SITE_UPDATED → RELEASE_STATE_UPDATED → VERIFIED → COMPLETED` 推进，注册表侧对应
§2.15.1 的 `DRAINING → REVOKED`；同一集群移除、加入再移除时，已 `COMPLETED` 的旧记录归档为
`state.completed-<时间戳>.json` 而不是当作 resume。`uninstall`（状态机见 §2.13）的选项
`--cpu-cluster keep|delete`、`--reset-database`、`--aurora-final-snapshot retain|skip` 由
`UninstallRequest.__post_init__` 约束：`skip` 只在 `delete` 或 `--reset-database` 下合法（普通重装
保留 Aurora，没有可快照的对象），`--reset-database` 只与 `keep` 组合。上述三者与 deploy/bootstrap、
Profile 审批、AdminConfig 竞争同一个 state-dir 变更锁。
...
- 作业身份：`attempt_observation`、`restart_budget`、`workload_coverage_heartbeat`。
- 修复预算：`remediation_budget_limits`、`remediation_budget_claims`、
...
- Processor 和周期任务兼容对象：`processor_request`、`processor_lane`、`processor_leadership`、
...
`create_postgres_schema()` 当前声明 68 个 `CREATE INDEX IF NOT EXISTS`
（55 个来自 `ddl.py`、9 个来自 `ddl_control_plane_review.py`、1 个来自
`ddl_processor_retry.py`、3 个来自 `ddl_spool.py`）、7 个 plpgsql 函数（`ddl.py` 5、
`ddl_spool.py` 1、`ddl_wakeups.py` 1）和 9 个触发器。v12 新增 6 个索引：
...
v13 新增 9 个（`ddl_control_plane_review.py`：`gpu_fault_workflow_updated_all`、
...
`gpu_fault_fleet_deployment_terminal`），并把热表 `gpu_fault_objects` 的
`autovacuum_vacuum_scale_factor`/`autovacuum_analyze_scale_factor` 设为 0.02/0.01（只在与现值
不同时才 `ALTER`）。v14 新增函数 `gpu_fault_objects_notify_wakeup()` 与触发器
...
| `gpu_fault_objects_notify_wakeup_trigger` | `FOR EACH ROW`（`AFTER INSERT OR UPDATE OF payload`） | workflow 进入可派发集合（新行，或排序/过滤字段变化）或 remote_command 状态迁移时 `pg_notify` 唤醒 dispatcher 与 executor claim；续租、取消请求这类同状态 UPDATE 保持安静，否则 dispatcher 会被自己的记账唤醒而反复重派 |
...
- 非活跃 marker、终态 notification（含 delivery、result 与去重 link）、completion record
  （decision + event）默认保留 2592000 秒（30 天），分别由
  `GPU_FAULT_MARKER_RETENTION_SECONDS`、`GPU_FAULT_NOTIFICATION_RETENTION_SECONDS`、
  `GPU_FAULT_COMPLETION_RECORD_RETENTION_SECONDS` 控制，非正值关闭该清扫；注册表 member
  心跳行保留 86400 秒（`GPU_FAULT_REGISTRY_MEMBER_RETENTION_SECONDS`）；
- incident、workflow、plan 不走裸 TTL，只走下文的 archive-first 保留。
...
站点默认保留 30 天（`src/gpu_fault/admin/site.py` 的 `DEFAULT_CONTROL_RECORD_RETENTION_DAYS`，
`site.yaml` 可改，显式 0 关闭）；应用侧 `GPU_FAULT_CONTROL_RECORD_RETENTION_DAYS` 的裸默认是
`0`=关闭，>0 时要求 PostgreSQL 与 `GPU_FAULT_CONTROL_RECORD_ARCHIVE_S3_URI`，归档桶由 deploy 的
`ensure_control_record_archive_bucket()`（`src/gpu_fault/admin/archive_bucket.py`）自动创建。
每小时由持有`control-record-archive`周期任务租约的单一副本扫描。
...
`POSTGRES_SCHEMA_MIGRATIONS` 当前有 **14 个版本**（v12 `store-review-hot-query-indexes`、
v13 `control-plane-review-indexes-and-autovacuum`、v14 `objects-wakeup-notify-trigger`），`LATEST_POSTGRES_SCHEMA_VERSION`
...
| `_validate_existing_schema()` | `GPU_FAULT_POSTGRES_AUTO_SCHEMA_INIT=false`，即三层控制面全部生产进程 | 只读校验：10 张关键 relation、4 个 plpgsql 函数（含 `gpu_fault_objects_notify_wakeup()`）、3 个 notify 触发器（含 `gpu_fault_objects_notify_wakeup_trigger`）、3 个优先级计数触发器都存在；`_validate_declared_indexes()` 要求 DDL 声明的每个索引存在、有效且定义未漂移（`CREATE INDEX CONCURRENTLY` 失败留下的 INVALID 索引和同名手建索引都不放过）；`_validate_triggers_enabled()` 拒绝任何被禁用的 `gpu_fault_*` 触发器；`_validate_counter_trigger_definitions()` 把六个计数触发器的表/函数/`tgtype`/transition table 与函数体逐字对照 DDL 声明（F-D10）；迁移历史逐行等于代码内注册表；`gpu_fault_schema_version.version` 等于 `LATEST_POSTGRES_SCHEMA_VERSION`；`dedicated` 热状态模式下再以 `hot_state_backfill_gaps()`（每类一条 `EXISTS`，不做 `count(*)`）确认 backfill 无缺口。任一不符抛 `RuntimeError`，提示先跑 `--ensure-schema` |
...
`create_app()` 当前始终挂载 **92 条应用路由**；区域模式
...
| `src/gpu_fault/app/routes/incidents.py` | 7 |
...
授权桶分布：`execution-token` 44、`cluster-token` 38、`dual-credential` 7、
`metrics` 1、`public` 2（模型见 §4.5）。清点路由必须用真实装配的 app（
...
| GET | `/livez` | public | 进程本地存活：事件循环应答、本角色不可或缺的消费线程活着；故意不看 Aurora——writer failover 只应让 Pod NotReady，而不是重启一个起来还要 Aurora 的进程 |
...
| GET | `/v1/incidents` | execution-token | 按 `state`（必填、可重复）与可选 `cluster_id`/`node_id`、`limit` 列出 incident 摘要，`updated_at` 倒序，`truncated` 标记截断；不给 `cluster_id` 时遍历注册表全部集群 |
| GET | `/v1/regional/registry/status` | execution-token | 持久注册表当前代、集群 lifecycle、成员 ack 与收敛状态（§2.15.1） |
| POST | `/v1/regional/registry/revisions` | execution-token | 以 `expected_generation` CAS 发布下一代注册表，不匹配 409 |
| POST | `/v1/regional/registry/rollback` | execution-token | 把 `target_generation` 的注册内容作为新一代重新发布 |
| POST | `/v1/regional/registry/clusters/{cluster_id}/transition` | execution-token | 单集群 `lifecycle_state` 过渡，最多 8 次 CAS 重试 |
...
### 4.2 集群侧事件入口（`cluster-token`，共 38）

以下 23 条是采集侧和节点侧入口，全部要求 `X-GPU-Fault-Cluster-ID` 与匹配的每集群
token；另外 15 条是 §4.4 的 executor 接口。
...
| POST | `/v1/attempts/coverage` | Completion Watcher 覆盖心跳（ROUTINE，latest_wins，见 §2.18.3） |
...
operator 侧（`execution-token`，27 条）：
...
共 **16 个端点**，全部在 `src/gpu_fault/app/routes/regional.py`。router 在非
...
重新分派。`{command_id}/progress` 是复合命令的逐步结果（§2.16），与续租同受 lease 围栏，但由
执行线程在每步落定时立即发出，不搭续租心跳的节拍。`fleet-rollout-fence` 让 executor 在执行
node-action 前读取本集群正处于 fleet rollout 的 deployment 集合做 fail-closed 围栏：控制面的围栏只
在派发前评估，收不回已写入 Store 的命令，而 executor 自身没有 Store 可查；集群取自认证头，不能
跨集群读。`regional_capture_evidence` 与 `regional_save_advisory_notification` 让
...
| 桶 | 判定 |
|---|---|
| `public` | 免鉴权。只有 `/healthz` 与 `/livez`，加上 `declared()` 兜底放行的 `/openapi.json`、`/docs`、`/docs/oauth2-redirect`、`/redoc` |
...
混合型号或显式配置冲突属于配置/硬件证据错误并阻止 Collector 启动；命令不可用时由
`GPU_FAULT_GPU_PRODUCT_DISCOVERY` 控制：`required` 拒绝启动，`auto` 回退到显式配置，
没有显式配置时不带产品启动（产品是批次上的身份元数据，不是读取宿主的前提；一个启动时
答不上来的驱动不能让整个宿主采集器 restart-loop 成「静默」节点）。
...
使用固定 `xid-not-applicable-zh-v2` 模板创建通知。模板字段来自 `XidEvent`、
...
`GET /metrics` 输出 Prometheus 文本。族数不写死在本文：`/metrics` 按角色与 Store 类型分段门控
（`src/gpu_fault/app/metrics.py`），核对时用 `create_app()` 渲染一次再按前缀统计。主要前缀：

| 前缀 | 覆盖内容 |
|---|---|
| `gpu_fault_processor_*` | 队列深度、claim、lane 等待/持有、延迟重试、已 claim 未开始、优雅停机释放、admission 与批处理、租约、清理与计数模式（最大的一组） |
| `gpu_fault_telemetry_*` | spool 深度、合并/取代、租约字节、重放耗时、通道新鲜度与通知回退 |
| `gpu_fault_workflow_*` | 派发周期/过滤/延后、内部错误、failure handling、抢占、预算拒绝、未 reconcile 的 BLOCKED |
| `gpu_fault_notification_*` | 投递总数/错误、outbox 深度、最老 pending、过期与 dead-letter、演练抑制 |
| `gpu_fault_remediation_budget_*` | 预算上限、活跃 claim、等待中的 workflow（全局/按集群/按 scope） |
| `gpu_fault_periodic_*` | 周期任务与清理的运行时间戳、错误、行数与预算耗尽 |
| `gpu_fault_ingress_*` | 每 lane 在飞数、等待时长、worker 数、背压与解码拒绝 |
| `gpu_fault_remote_command_*` | 命令状态与按集群计数、最老/过期未领取、执行器内部错误、复合命令与批步骤计数、sibling hold |
| `gpu_fault_request_decode_*` / `gpu_fault_store_io_*` | 解码 / Store I/O 的并发、等待与拒绝 |
| `gpu_fault_metrics_aggregation_*` | 多进程合并本身：合并到的进程数与 degraded 标志 |
| 其他单例 | event-loop lag、PostgreSQL pool checkout、未知产品、attempt 归属歧义、stale Agent、闭环 milestone、incident 按状态计数、控制记录归档 |

多进程 Pod 的 `/metrics` 合并（`src/gpu_fault/app/process_metrics.py`）：ingress 与 control-worker
以 `--workers 4` 运行，每个 uvicorn 进程各有一份进程内计数器，一次 scrape 只命中其中一个进程，
同一 Pod 的连续 scrape 会在不同进程指纹之间跳动、计数读成 0。因此每个进程把自己完整的渲染写到
`/dev/shm/gpu-fault-process-metrics/<pid>.json`（每次应答时，以及每 5 秒由
`gpu-fault-process-metrics` 线程），应答进程把自己的新鲜渲染与其他存活进程的文件合并，每个族按
`metric_aggregation` 登记的策略聚合（SUM/MAX/MIN/ANY/PER_PROCESS；PER_PROCESS 族带
`process="<slot>"` 标签，slot 由 `flock` 占用、进程死亡即释放）；PID 已消失的文件当场删除，
Prometheus 把随之减少的和读成一次 counter reset。目录默认由 `POD_UID` 派生，
`GPU_FAULT_PROCESS_METRICS_DIR` 可覆盖或以 `off` 关闭；单进程与单元测试只合并自己一份，字节等于
普通渲染加两个合并器 gauge。
...
`make docs-check` 本身是十项生成物/引用一致性检查（`docs-static-check`：doc-impact、impact、
env-doc、case-index、manual-command-order、doc-reference、doc-anchor、fault-evidence、doc-facts、
grafana-dashboards）加 `DOCUMENTATION_TESTS` 列出的 12 个文档契约测试文件（含
`tests/test_doc_impact.py` 与 `tests/test_change_impact.py`）。`doc-anchor` 解析每篇公开
