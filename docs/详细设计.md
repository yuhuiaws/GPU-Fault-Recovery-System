...
`src/gpu_fault/admin_resource_registry.py` 管理生产部署产生或接管的 AWS 资源。
...
`create_postgres_schema()` 当前创建 51 个 `CREATE INDEX IF NOT EXISTS`
（47 个来自 `ddl.py`、1 个来自 `ddl_processor_retry.py`、3 个来自
`ddl_spool.py`）、6 个 plpgsql 函数和 8 个触发器：
...
`POSTGRES_SCHEMA_MIGRATIONS` 当前有 **7 个版本**，`LATEST_POSTGRES_SCHEMA_VERSION`
...
`create_app()` 当前始终挂载 **84 条应用路由**；区域模式
...
`GET /metrics` 输出 Prometheus 文本。按区域 worker 角色装配真实 app 后，以 `# HELP`
计数为 **149 个指标族**：
...
`tests/` 共 223 个 Python 文件，规模最大的四组是 `tests/regional/`（43）、顶层
`tests/`（25）、`tests/orchestration/`（19）、`tests/hyperpod/`（12），其余覆盖
...
`GPU_FAULT_TEST_POSTGRES_URL` + `make test-postgres`。锁竞争压力档使用
...
fault-evidence）加 10 个文档契约测试文件。生成类文档
