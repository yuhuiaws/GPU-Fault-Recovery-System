...
`cleanup_job_errors_total{job}` 等指标（F-F2）；周期任务体本身抛异常则计入
...
- **fail-forward**：`upgrade_release` 的失败分支在 state 有接受记录时不调用 `rollback`；
  `scripts/release_deploy.py` 镜像同一判断，把 `SKIPPED_POLICY` 的 reason 写成 schema 接受
...
| `GPU_FAULT_NODE_ACTION_RETENTION_SECONDS` / `_MAX_RESULTS` | `2592000` / `10000` | 账本保留（按 `(command_id, attempt)` 逐次追加；schema `PRAGMA user_version=3`，审计列含 `agent_generation`——接单时 Agent 自身的实时代际，v3 之前的行与首个心跳成功前接下的命令为 NULL；旧账本打开时就地迁移，v2 只加列不搬表；全表见 `node_agent/ledger.py`） |
...
从 `node_installer_reconciler` 的实例类型清单渲染（带与不带 `ml.` 前缀各一份），
...
`GPU_FAULT_INGRESS_DECODE_WORKERS`（8 线程、in-flight 128、5s 超时），故障路径
（`/v1/gpu-events/*`、`/v1/provider-events/*`、`/v1/attempts/*` 等 FAULT 前缀）走独立的
`GPU_FAULT_FAULT_DECODE_WORKERS` 池，遥测洪峰不再挤掉故障事件的解码；`cluster_id`
...
`UNKNOWN`，这正是期望的 fail-closed。心跳每 15s 一次，后续可把 freshness 收紧到
120–180s 而不改变语义。`resolve` 读的是**有界的 newest-first 窗口**
...
`gpu_fault_cluster_executor_*`，27 条：计数器 20 条——`claims_total`、`results_succeeded_total` /
...
| 表 | 主键 | 内容 |
|---|---|---|
| `gpu_fault_objects` | `(kind, key)` | 通用 JSONB 文档表；Store codec 当前注册 48 种逻辑 kind（其中 `diagnostic`、`triage` 两种仅为清理 cutover 前旧行而保留），包含故障闭环、通知、fleet/区域、安装资源、遥测、Processor 与策略状态 |
...
48 种 codec kind（46 种在用，`diagnostic`、`triage` 仅供清理旧行）大致分为：控制/工作流 8 种（event、decision、marker、plan、
profile、incident、workflow、restart_budget）；通知 4 种；
fleet/区域/安装生命周期 8 种（含 `installation_resource`）；遥测与健康 12 种；
Processor/租约 4 种；策略、XID 与 EFA 关联状态 7 种。该数字表示 Store 可解码的
逻辑类型，不表示生产 `dedicated` 模式下 48 种都会在 `gpu_fault_objects` 中有行：
GPU 热状态和 Processor queue/lane 会落到专表，通用表只保留兼容或低频对象。

索引与触发器总量：**59 个 `CREATE INDEX IF NOT EXISTS`**（55 在
...
| `incident` / `workflow` | **无 TTL** | 由 `control-record-archive` 归档，见下；打开归档后按 `GPU_FAULT_CONTROL_RECORD_RETENTION_DAYS`（生产 30 天）回收 |
...
`SERIALIZABLE` 事务把超过 `GPU_FAULT_CONTROL_RECORD_RETENTION_DAYS`（生产 30 天；0 即关闭，
并要求同时给出 `GPU_FAULT_CONTROL_RECORD_ARCHIVE_S3_URI`）的 incident 先写归档目的地、
...
`gpu-fault-admin workflow-reconcile`只有一种模式（`--dry-run`只打印计划，不带它则一次
完成计划与应用），接受两种形状：**已验证恢复**——同
...
- 注册表在 `src/gpu_fault/schema_migrations.py`，每项是
  ...
  **13 个版本**；v7 是 `processor-retry-schedule-and-lane-policy`，v12 是
...
| 路由模块 | 条数 | 干什么 |
|---|---:|---|
| `src/gpu_fault/app/routes/regional.py` | 15 | 集群执行器全部交互（claim/renew/progress/result、证据、就绪、spare 健康、HyperPod 提交、drain/revoke） |
...
| `gpu_fault_remote_command_*` | 5 | 命令总数、最老未认领时长、过期未认领、执行器内部错误、`gpu_fault_remote_command_open_sibling_holds_total`（同一 workflow 步骤已有未关闭命令而被持有的派发，进程级 counter，ARCH-D5） |
...
2026-09-07 架构评审之后追加的族（不计入上表的 151）：
...
路径/符号存在、生成物同步、命令顺序。本文里的数字（149 个指标族、84 条路由、154 个区域
用例、48 个 Store codec kind、19 类通知）都是通过装配真实 app、实例化 Store codec 或读取
机器源重数得到的，改代码后要重数，没有守卫会替你发现它们过期。
...
| 建议 | 实现 |
|---|---|
| GPUDevice / FaultRule / RemediationPolicy / RemediationRun 各自建表 | 低频对象落在 `gpu_fault_objects (kind, key) → JSONB`；Store codec 注册 48 个逻辑 kind，GPU 热状态与 Processor queue/lane 在生产 dedicated 模式走专表（§4.4） |
