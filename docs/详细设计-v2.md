...
- **发布引擎的接入**：`bootstrap_gpu_target` 与 `join_cluster` 在 DCGM exporter 之后应用采集器；升级计划的
  ...
  而是在 observability 快照恢复阶段之后由 `rollback_gpu_adot_collectors` 用 `previous["adot_image"]` 重新应用
  每个集群的采集器。`observability_adot` 摘要 =
  ...
  会因公式变化移动一次。缺失告警 `GpuFaultDataplaneCollectorMissing`（`absent(up{job="gpu-fault-dataplane"} == 1)`
  或某个 `gpu_cluster` 的 `max(up) == 0` 持续 15 分钟，warning）；两条 Completion Watcher 规则按
  `gpu_cluster` 分组。已知盲区：多集群时一个采集器死、另一个活，前者没有任何序列，两半都抓不到。
