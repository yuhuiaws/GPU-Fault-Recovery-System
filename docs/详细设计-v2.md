...
三个前提要写进部署判断：（1）「作业」的定义就是带 `gpu-fault.io/managed=true` 的
Pod——只跑未打标负载的集群会被读成 `IDLE`，这是本特性唯一一条没有真实覆盖的
`UNKNOWN → IDLE` 通路，接入这类负载前必须先打标；（2）
`GPU_FAULT_COMPLETION_OBSERVE_UNMANAGED=true` 时 selector 为空、全集群 Pod 都被计数，
任何 Running 的系统 Pod 都会压住心跳，该模式下心跳事实上关闭（安全方向）；（3）心跳
把 `IDLE` 交给无 checkpoint 的 REBOOT/RESET 路径，所以它宁可少发也不能多发。
