...
- 三种长时可变更的重装操作（`REMEDIATE_DRIVER`、`UPDATE_SOFTWARE_FIRMWARE`、
  ...
  开事务之前各读一次控制面 store（探针 `inflight_installs`，先走 CPU ingress Pod，失败再试其它有 Running Pod
  的控制面角色）。探针只扫可执行状态 `PENDING` / `SAFETY_PENDING` / `RUNNING`。BLOCKED 不扫，依据不是「BLOCKED 持不了
  ...
  **已派发的 BLOCKED 行永不再执行**：执行器对 BLOCKED 直接返回既有结果，运维杠杆只会把它关到 SUPERSEDED 且拒绝带
  ...
  超界以「could not bound the scan」拒绝而不是读作 0。门按证据分两类失败：**探针没能运行**（没有任何 Running 控制面 Pod，或 kubectl 传输失败且没有探针启动的痕迹，
  `StoreUnreachable`）与**证据缺陷**（探针跑起来后非零退出、没有输出 JSON、自报错误、超时、形状不对、扫描超界）。
