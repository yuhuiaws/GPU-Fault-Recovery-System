...
| 节点 | 5 个可安装 collector systemd 单元（生产默认启用 4 个，NodeLog 默认禁用） | systemd | 每节点各 1 | `/opt/gpu-fault/venv/bin/gpu-fault-collector <mode>` | 无监听端口 | root（见 §2.4） |
| 节点 | `gpu-fault-node-agent` | systemd | 每节点 1 | `/opt/gpu-fault/venv/bin/gpu-fault-node-agent` | **9099**；裸默认 TLS，当前区域安装显式私网 HTTP | root |
...
| 3.13 | Installation Resource Registry | 管理 CLI + 控制面 API | `src/gpu_fault/installation_resources.py`、`src/gpu_fault/admin_resource_registry.py` |
...
| `dual-credential` | 7 | 二选一：带集群头时用 cluster token，否则用 execution token | 既可能被数据面也可能被运维调用的接口（fleet 查询、drain/revoke、GPU 指标读取） |
...
### 10.2 指标：真实有 149 个族，但建议的 7 个名字一个都不存在
...
84 条路由、153 个区域用例、45 个 Store codec kind、19 类通知）都是通过装配真实 app、
