...
- 三种长时可变更的重装操作（`REMEDIATE_DRIVER`、`UPDATE_SOFTWARE_FIRMWARE`、
  ...
  的控制面角色）。探针只扫可执行状态 `PENDING` / `SAFETY_PENDING` / `RUNNING`（BLOCKED 不可能持有 WAITING 的
  重装步骤，不扫），未完成、未 superseded 的重装步骤无执行记录为 `PENDING`、最新执行 WAITING 为 `WAITING`，
  计划任意深度、仍在 SAFETY_PENDING 的都算，在该步骤整个生命周期内挡住每一次发布；扫描以 1000 行为界，
  超界以「could not bound the scan」拒绝而不是读作 0。手工入口 fail closed：store 读不到同样拒绝，杠杆按入口分
  （`gpu-fault-admin deploy --allow-inflight-installs`；`rollout-regional-release.sh rollback` 用环境变量
  `GPU_FAULT_RELEASE_ALLOW_INFLIGHT_INSTALLS=1`），带杠杆仍尽量读并记
  `inflight-installs-overridden` / `inflight-installs-unchecked`。自动回滚（`rollback_release(automatic=True)`，
  引擎 `rollback --automatic`）在 store 读不到时记 `inflight-installs-unchecked` 后继续，但真有重装在飞时照样拒绝，
  事务停在 `failed`；引擎退出码 3，驱动记 `REFUSED_INFLIGHT_INSTALLS`。
