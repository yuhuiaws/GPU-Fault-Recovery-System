...
（`GPU_FAULT_NOTIFICATION_RETENTION_SECONDS`；PENDING/RETRY/LEASED 永不清）、已过
PENDING_TRIAGE 的 completion 决策（`GPU_FAULT_COMPLETION_RECORD_RETENTION_SECONDS`），
...
- **诊断类 marker 不拥有作业恢复**：`COLLECT_EVIDENCE` / `RUN_DIAGNOSTICS` /
  ...
  拥有者，terminal 照常走 quick triage；marker 本身保持 active，仍是有效观测。没有存到
...
| 非活跃 `marker` / 终态 `notification` / 已过 PENDING_TRIAGE 的 completion 决策 | 2,592,000s（30 天），仍被 incident 引用则保留 | `processor-cleanup` 的 `inactive_markers` / `notifications` / `completion_records` job（§2.3.1） |
