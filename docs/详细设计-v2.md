...
| 节点 OS | 需要 systemd、`journald`、`/dev/kmsg`、`nvidia-smi`；节点侧装在 `/opt/gpu-fault/venv` 独立 venv 里 | `deploy/systemd/`、`deploy/node/install-gpu-fault-collector.sh` |
...
| 场景 | 现状 | 依据 |
|---|---|---|
| 调用 `BatchReplaceClusterNodes` 替换节点 | 恒禁用。`GPU_FAULT_ALLOW_HYPERPOD_REPLACE=false` 是数据面执行器上的第二道锁，第一道在 spare 协调分支 | `deploy/dataplane/cluster-action-executor.yaml` 注释与 `src/gpu_fault/adapters/hyperpod/lifecycle.py` |
...
一次完整部署有 **11 类长驻进程**（含 0 副本的泄压角色），分布在三层。所有控制面与
数据面容器都是同一个 `gpu_fault_control_plane-0.10.0-py3-none-any.whl`，从
ConfigMap 卷 `/artifact` 里 `pip install` 后启动不同入口——镜像里没有业务代码，
因此"改代码"等于"换 wheel ConfigMap 并滚动所有引用它的工作负载"。
...
| 数据面 | `gpu-fault-metrics-collector` | DaemonSet | 每 GPU 节点 | `gpu-fault-collector dcgm` | 无监听端口 | 默认 |
...
| 节点 | 5 个 collector systemd 单元 | systemd | 每节点各 1 | `/opt/gpu-fault/venv/bin/gpu-fault-collector <mode>` | 无监听端口 | root（见 §2.4） |
| 节点 | `gpu-fault-node-agent` | systemd | 每节点 1 | `/opt/gpu-fault/venv/bin/gpu-fault-node-agent` | **9099/TLS** | root |
...
|---|---|---|---|---|
| `ingress` | 只做鉴权、解码、准入、**入队**，并返回回执 | 只有 `gpu-fault-collector-metrics-snapshot` | `--backlog 8192 --limit-concurrency 4096 --limit-max-requests 20000 --limit-max-requests-jitter 2000` | 4 CPU / 4Gi → 8 CPU / 8Gi |
...
三个角色的 12 个 ConfigMap（每角色 core/notification/postgres/processor/recovery/telemetry）
由渲染器生成，手改会被 `make docs-check` 的清单检查拦下。改环境变量前必须先确认它
不是 `valueFrom`——生成清单里 16 个变量来自 Secret/ConfigMap 引用，直接改 `value` 不生效。
...
| 路径 | 内容 | 权限 |
|---|---|---|
| `/opt/gpu-fault/venv/` | 独立 venv，含 `gpu-fault-collector`、`gpu-fault-node-agent`、可选 `py-spy` | 0755 |
...
- PDB：worker 与 spool-worker 各有 `maxUnavailable: 1`。**ingress 没有 PDB**。
...
sequenceDiagram
    ...
    Wk->>Wk: 启动 §2.3 全部线程
    Exe->>Exe: pip install（4 次退避重试）→ touch /tmp/executor-ready
...
- 执行器的 `startupProbe` 只证明 `pip install` 跑完了。**Ready 由
...
3. `stop` 事件置位 → `processor.stop()` → 逐个 `join` 各线程，超预算记录
   `processor shutdown deadline exceeded` 并把失败线程名写日志；
...
建议大纲列了 10 个模块，其中"AMD / Intel Adapter"没有实现（§11），而实现里另有两个
必须写清楚的承重模块：**Cluster Action Executor**（数据面唯一持 AWS 凭据、唯一直接
操作 Kubernetes 的进程）与 **Processor**（所有写入的统一入口与租约层）。本章覆盖
12 个模块，每个模块固定 12 段：
...
`GPU_FAULT_NODE_AGENT_ALLOW_PLAINTEXT=true` → 校验 `GPU_FAULT_PROC_ROOT` 能看到
...
| `GPU_FAULT_NODE_AGENT_TLS_CERT` / `_TLS_KEY` / `_TLS_CLIENT_CA` | — | 证书与私钥必须成对；给了 client CA 就要求 mTLS；缺证书时只有显式允许 plaintext 才启动 |
| `GPU_FAULT_NODE_AGENT_ALLOW_PLAINTEXT` | 裸默认 `false`；当前区域生产安装为 `true` | 无服务端证书时必须显式打开；当前使用节点私网 HTTP，后续切换 TLS 时应改回 false |
...
`restore_failed`，并把
还原状态留在 `/var/lib/gpu-fault/quiesce/` 供人工收尾。
...
| 门 | 阈值 | 行为 |
|---|---:|---|
| 单请求体上限 | `GPU_FAULT_PROCESSOR_MAX_REQUEST_BYTES=16777216` | 超限 413 |
...
`GPU_FAULT_HYPERPOD_MANAGED_RECOVERY_TIMEOUT_SECONDS=2700`、
...
**注意：没有集群级或机架级的并发上限**——只有每执行器 `GPU_FAULT_CLUSTER_EXECUTOR_MAX_CONCURRENT_COMMANDS=5`、
...
**⑤ 启停流程**——见 §2.7：`pip install`（4 次退避重试）→ `touch /tmp/executor-ready`
→ 校验 IRSA 与 `CONFIRM_CLUSTER` → readiness 由控制面回答。停机时归还租约。
...
`delivery_status`、`attempt`。**`/metrics` 里完全没有通知类指标**（§10.2）——
...
而不是挂着半死）。`GPU_FAULT_PROCESSOR_THREAD_DUMP_SIGNAL=SIGUSR2` 可现场取线程栈。

**⑥ 依赖服务**——Aurora（含 `LISTEN/NOTIFY`）。

**⑦ 超时/重试/熔断**——请求租约 150s / 续租 10s / 最长执行 120s / 响应超时 115s；
可重试响应最大年龄 300s；空闲退避最大 2s（FAULT 池 0.5s）、忙时退避 0.1s；
过载返回 429 + `Retry-After: 2`。

**⑧ 幂等与并发控制**——`lease_owner` + `leader_epoch` + 随机 `lease_token` +
`lease_expires_at` 四元组，配 `FOR UPDATE SKIP LOCKED`；完成的请求保留 600s 供
重放同一响应（这就是"客户端重投拿到同样回执"的机制）。

**⑨ 配置参数**——见 §3.3 ⑦ 与本节 ④⑤⑦；清理见 §2.3.1。

**⑩ 日志/指标/告警**——`processor_request_id`、`path`、`priority`、`cluster_id`、
`lane`、`lease_owner`、`leader_epoch`、`attempt`、`response_status`、`duration_ms`。

**⑪ 异常处理**——`StoreIoCapacityExceeded` → 503；队列深度超限 → 429；
租约丢失后写入 → 409；处理异常 → 记录失败响应并按可重试性决定重投。
...
必填只有 5 个：`cluster_id`、`node_id`、`observed_at`、`xid`，以及 `event_id`
...
| `RemediationRun` | `WorkflowRequest`（34 字段）+ `WorkflowStepExecution` 列表 + `RecoveryPlan` | `src/gpu_fault/models.py` |
...
| 表 | 主键 | 内容 |
|---|---|---|
| `gpu_fault_objects` | `(kind, key)` | **26 种 `kind` 的 JSONB 文档**：`incident`、`workflow`、`marker`、`notification`、`notification_delivery`、`notification_result`、`remote_command`、`agent`、`barrier`、`collector_status`、`fleet_deployment`、`gpu_finding_state`、`gpu_finding_history`、`gpu_inventory_snapshot`、`gpu_metric_latest`、`gpu_metrics_batch`、`health_signal_state`、`hyperpod_node_identity`、`processor_request`、`raw_evidence`、`regional_cluster`、`telemetry_metric_latest`、`training_progress`、`attempt_observation`、`xid_correlation`、`xid_correlation_event` |
...
| `gpu_fault_processor_queue` | `request_id` | 请求队列 + 租约 + 回放响应 |
...
索引与触发器总量：**50 个 `CREATE INDEX IF NOT EXISTS`**（47 在
`src/gpu_fault/store/postgres/ddl.py`，3 在 `src/gpu_fault/store/postgres/ddl_spool.py`）、
...
- 注册表在 `src/gpu_fault/schema_migrations.py`，每项是
  `SchemaMigration(version, name, ddl_checksum, apply, legacy_checksum)`，当前 **6 个版本**。
...
- 迁移是**只进不退**的：没有 down migration。回滚靠部署上一版 wheel，前提是新版本的
  DDL 对旧代码向后兼容——所以新增列必须 `ADD COLUMN IF NOT EXISTS` 且可空。
...
失败重来（§9.7），还原失败会留下 quiesce 状态文件供人工收尾
...
sequenceDiagram
    ...
    Note over D,Ex: 并发只受每执行器 5 个命令 + barrier + wave + 重启预算约束，<br/>没有集群级或机架级并发上限
```
...
（清点方法见 §0.3）。区域模式下 `create_app()` 挂出 **81 条业务路由**
...
| `execution-token` | 36 | `X-GPU-Fault-Execution-Token` | 运维与控制面自身 |
| `dual-credential` | 7 | 上述两者**同时**提供 | 既可能被数据面也可能被运维调用的接口（fleet 查询、drain/revoke、GPU 指标读取） |
| `metrics` | 1 | 仅回环 | `GET /metrics` |
...
请求丢了**，凭 `processor_request_id` 去 `GET /v1/processor/requests/{id}` 查。
...
| `src/gpu_fault/app/routes/telemetry.py`（metrics 模块） | 2 | `/metrics`、`/v1/internal/metrics/collector-silence` |
| `src/gpu_fault/app/routes/processor.py` | 2 | 队列请求收据查询、队列状态 |
| `src/gpu_fault/app/routes/configuration.py` | 2 | `/v1/markers`、`/v1/runtime-profiles` |
...
| GET | `/v1/version` | execution-token | 版本与 `module_digest` |
...
显式设置 `GPU_FAULT_NODE_AGENT_ALLOW_PLAINTEXT=true` 并广告节点私网 HTTP 地址：
...
| 重放触发 | **只在一次成功投递之后** | 投递失败说明链路还断着，重放纯属浪费 |
| 重放顺序 | **实时事件先发，积压后补**——不是 FIFO | 否则一条新 XID 会排在最多 10 条陈旧记录后面，每条都可能烧掉 `max_attempts` 次超时与一次收据轮询，刚恢复网络的节点会把自己的故障上报推迟数分钟 |
| 重放批量 | `outbox_replay_batch_size=10` + `GPU_FAULT_COLLECTOR_OUTBOX_REPLAY_BUDGET_SECONDS=5` 墙钟预算 | 每条只试一次、不轮询收据 |
...
**必须明确：实现里没有集群级或机架级的修复并发上限。** 建议里的
"cluster-/rack-level concurrency limit" 不存在，也没有对应配置项。实际约束修复面的
是四个别的机制：

| 机制 | 参数 | 约束什么 |
|---|---|---|
| 每执行器在飞命令数 | `GPU_FAULT_CLUSTER_EXECUTOR_MAX_CONCURRENT_COMMANDS=5` | 单个集群执行器同时下发的命令数（**不是节点数上限**） |
...
设计上的取舍：barrier + wave 控的是"什么时候一起动"，比一个全局并发数更贴合
fabric 域的语义；但**它不防"同时有很多互不相关的节点各自在修"**。如果站点需要那种
全局刹车，现在只能靠关 dispatcher 开关（§7.7）。这是一个已知缺口，见 §11。
...
| 面 | 机制 |
|---|---|
| 集群身份 | 每集群一个 token，库里只存 `token_sha256`（64 hex），比较用 `secrets.compare_digest`；token 最短 32 字符 |
...
| 传输 | Node Agent 应用裸默认 `NODE_AGENT_ALLOW_PLAINTEXT=false`；当前区域生产安装显式允许节点私网 HTTP。配置 TLS 时，证书剩余有效期低于 `CERTIFICATE_MIN_VALIDITY_SECONDS=2592000`（30 天）即告警 |
...
- **集群 token**：`RegionalClusterRegistration` 只有**一个** `token_sha256`，**没有
  双 token 重叠窗口**。轮转是切换式的：改注册与改数据面 Secret 之间存在一个短窗口，
  旧 token 会 401。要无缝就得先注册新集群 ID 再迁移，或选在低峰期切。这是已知限制。
...
`GPU_FAULT_LOG_LEVEL` / `LOG_LEVEL` 控制（默认 INFO）。没有 JSON 日志器，字段以
`key=value` 内嵌在 message 里。`_configure_logging()` 在根 logger 已有 handler 时
不接管（让 pytest 与嵌入方保留控制）——这也是历史上"配置错了却一片安静"的原因：
在它存在之前 uvicorn 只配 `uvicorn*` logger，控制面自己的 `LOGGER.info` 全被丢弃。
...
### 10.2 指标：真实有 143 个族，但建议的 7 个名字一个都不存在

`GET /metrics`（Prometheus 文本格式，仅回环）在区域模式下导出 **143 个指标族**，
全部以 `gpu_fault_` 开头，分布是：

| 前缀 | 族数 | 覆盖 |
|---|---:|---|
| `gpu_fault_processor_*` | 83 | 队列深度、lane、租约、消费、清理、通知（指 PostgreSQL `pg_notify`）、优先级计数 |
...
| 其他 4 个单例 | 4 | `gpu_fault_event_loop_lag_seconds`、`gpu_fault_postgres_pool_checkout_wait_seconds`、`gpu_fault_policy_unknown_product_total`、`gpu_fault_ambiguous_attempt_ownership_total` |
...
| `remediation_failures_total` | ✗ | `gpu_fault_remote_command_executor_internal_errors_total`（只覆盖执行器自身缺陷，不是修复失败） |
...
**明确尚未导出的指标**（写出来是为了让人别去 Grafana 里找）：

- workflow 结果（成功/失败/抢占计数与耗时分布）
- dispatcher 待办积压
- 通知投递结果与积压——`/metrics` 里**完全没有通知相关指标**，通知侧异常只能看日志
- 失联 Agent 数（心跳新鲜度分布）
- 采集器新鲜度的**聚合视图**（单集群可查 `GET /v1/collector-readiness/{cluster_id}`，但没有对应指标族）
- API 时延直方图（只有 `X-GPU-Fault-Server-Duration-Ms` 响应头）
...
| `make docs-check` | `doc-impact-check`、`env-doc-check`、`case-index-check`、`manual-command-order-check`、`doc-reference-check`、`fault-evidence-check`，再跑文档契约测试集 |
| `make coverage` | 4 worker 并行，分支覆盖，`--cov-fail-under=78` |
| `make test-postgres` | 需要 `GPU_FAULT_TEST_POSTGRES_URL`；不给就直接退出 2 |

测试规模：`tests/` 下 198 个 Python 文件。**`make docs-check` 不校验文档断言与代码
一致性**——它只查引用路径/符号存在、生成物同步、命令顺序。本文里的数字（143 个指标族、
81 条路由、147 个用例、26 个 kind、19 类通知）都是我装配真实 app 或读真实文件数出来的，
...
| 集群级 / 机架级修复并发上限 | **没有**（§9.5） | 真缺口。当前只能靠关 dispatcher 全局刹车 |
...
| 建议 | 实现 |
|---|---|
| GPUDevice / FaultRule / RemediationPolicy / RemediationRun 各自建表 | 全部落在 `gpu_fault_objects (kind, key) → JSONB` 的 26 个 kind 里（§4.4） |
...
| workflow / 通知 / 失联 Agent 指标 | 未导出（§10.2 清单） |

### 11.5 优先级建议

按"影响面 × 改动成本"排，前三项值得先做：

1. **集群级修复并发上限**（§9.5）——现在唯一的全局刹车是关开关，风险最高。
2. **workflow 与通知指标**（§10.2）——不加就无法回答"最近一小时修了几台、成功几台"。
3. **approve 端点**（若站点要求人工放行）——现在的替代路径（改 pin / 关开关）是运维
   动作，不可审计、不可授权到人。
...
| [区域模式端到端验收测试用例.md](区域模式端到端验收测试用例.md) | 现场验收规格，147 个用例 |
