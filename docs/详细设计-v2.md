...
- **覆盖心跳**：Watcher 每次**全量**扫描结束后 POST `/v1/workload-coverage`
  （`WorkloadCoverageHeartbeat{cluster_id, scanned_at, attempt_count, observe_unmanaged}`），
  按 `GPU_FAULT_WORKLOAD_COVERAGE_HEARTBEAT_SECONDS`（默认 15s）节流；watch 事件触发的
  过滤扫描只看被触碰的 attempt，不发心跳。它是 fire-and-forget：发送失败只记日志、不进
  outbox，下一次扫描自然覆盖。空闲集群没有托管 attempt 因而没有任何 `AttemptObservation`，
  控制面此前分不清「没有作业」与「watcher 已死」，只能 fail closed 到 `UNKNOWN`；心跳把
  「集群被扫过」本身作为事实交给控制面（§3.4 ⑦）。
...
| `GPU_FAULT_NODE_ACTION_RETENTION_SECONDS` / `_MAX_RESULTS` | `2592000` / `10000` | 账本保留（按 `(command_id, attempt)` 逐次追加，审计列见 `node_agent/ledger.py`） |
...
**任何 attempt 观测或一条覆盖心跳**（§2.7，`get_workload_coverage`）且没有一条观测命中
...
（`src/gpu_fault/cluster_executor_batching.py`）：每一步都用**它自己的** `step_index`/
...
每步的异常按与单步命令**相同**的分类（`cluster_executor_results.py`）落到该步自己的条目上。
...
| `gpu_fault_ingest_*` 追加 | `gpu_fault_ingest_unresolved_fault_signals_total{kind}`，kind ∈ `unparsed_xid_line`/`unparsed_sxid_line`/`unclassified_sxid`；`fault_ingestion` 未绑定到上下文时不输出（G7） |
