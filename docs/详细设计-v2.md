...
- **写到同一个 AMP workspace**：`prometheusremotewrite` + `sigv4auth`，凭证来自 ServiceAccount
  `gpu-fault-adot-dataplane` 的 IRSA 注解（`aps:RemoteWrite` 限定到该 workspace ARN）；
...
- **发布引擎的接入**：`bootstrap_gpu_target` 与 `join_cluster` 在 DCGM exporter 之后应用采集器；升级计划的
  ...
  规则命名空间的内容），控制面一半仍在 `observability_restore` 阶段恢复（记录 `control_plane_collector:
  restored|restored-unchanged`、`dataplane_adot: deferred to dataplane_observability_restore`）；数据面一半是独立阶段
  ...
  重跑）；最后恢复或删除规则
  命名空间（存在性以 `ResourceNotFoundException` 判定，其它 describe 失败抛错；非 ACTIVE 状态先等待稳定，5 s × 60；安装脚本删除后
  等到 ResourceNotFound；恢复端 put/create 之后不等 ACTIVE）。整份快照——两个平面——先校验再动任何对象；阶段返回的记录由阶段运行器持久化
...
`gpu_fault_cluster_executor_*`，14 条：计数器 `claims_total`、`results_succeeded_total` /
...
`transport_retries_total`；gauge `in_flight_commands`、`claim_loop_alive`、`stuck_executions`；
时间戳 `last_claim_timestamp`、`last_loop_iteration_timestamp`）。`increment` 是执行器计数器的唯一
写入口并同步镜像到这一族，所以 scrape 与 readiness breadcrumb 读到的是同一组数。读法：
...
- 三种长时可变更的重装操作（`REMEDIATE_DRIVER`、`UPDATE_SOFTWARE_FIRMWARE`、
  ...
  **已派发的 BLOCKED 行永不再执行**：执行器对 BLOCKED 直接返回既有结果，运维杠杆只会把它关到 SUPERSEDED 且拒绝带
  ...
  探针没能运行时手工入口同样拒绝，杠杆按入口分（`gpu-fault-admin deploy --allow-inflight-installs`；
...
| `gpu_fault_cluster_executor_*` | 9111 | executor，14 条（见 §3.9 ⑩） |
