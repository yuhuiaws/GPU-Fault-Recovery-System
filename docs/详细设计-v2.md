...
两者的 `details` 都带 `execution_timeout: true`、`outcome_unknown: true`、
`execution_timeout_seconds` 和 `operation`；转发过 node action 时还带
`node_action_command_id`，那是操作员唯一能在 Agent 账本里查到真实结果的入口。
