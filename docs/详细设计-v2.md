...
`gpu-fault-admin workflow-reconcile --plan/--apply`先校验同incident已`RECOVERED`、
successor为`SUCCEEDED`且完成`RESTORE_SCHEDULING`，再校验fencing、lease、open remote
command、WAITING provider动作、FAILED source plan以及GPU节点无残留cordon/quarantine。
apply重新计算计划摘要，并在Store事务中同时把predecessor转为`SUPERSEDED`、保留incident
指向successor、给incident和source plan增加恢复关联；记录仍留在live store，之后才可由
上述archive-first流程归档。
...
- `gpu_fault/retired_generation.py`持有判定，`WorkflowDispatcher`每轮扫一次，**先取消remote
  command再终态化**。任何无法证明的情况都保留阻塞语义：已完成的destructive操作、`LEASED`
  command、本地`WAITING`步骤都交给管理员，不自动清理。
...
两个模式的计划摘要都**不包含**`workflow_updated_at`。退役世代正在被循环派发，每tick都会
续租并重写该时间戳；把它计入摘要会让管理员刚审完的计划必然作废——2026-09-04
`workflow-45c6b6b7`的apply就因为两次求值之间只差19:25:59→19:26:54而被拒。摘要仍覆盖全部
决策输入（status、两个fencing token、incident状态、successor身份与世代、已完成/待执行
destructive操作、未结算本地步骤、budget claim、open command），写入本身则由事务内的
fencing token比较守卫；时间戳照常打印，因为"几秒前刚被碰过"正是管理员判断这是活的楔子
而非历史遗留的依据。
