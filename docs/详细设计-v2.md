...
| 前缀 | 覆盖 |
|---|---|
| `gpu_fault_processor_*` 追加 | `gpu_fault_processor_fault_rejections_total`（入口 202 之后回放被 4xx 的故障层事件）、`gpu_fault_processor_completions_by_path_status_total{path,status_class}`（G1）；`gpu_fault_processor_claim_last_round_timestamp_seconds` 仅在处理器快照带该键时输出（E3） |
...
| `gpu_fault_ingest_*` 追加 | `gpu_fault_ingest_unresolved_fault_signals_total{kind}`，kind ∈ `unparsed_xid_line`/`unparsed_sxid_line`/`unclassified_sxid`/`hma_unschedulable_without_code`（HMA 把节点标为 Unschedulable 但未给出可解析 XID/SXID）；`fault_ingestion` 未绑定到上下文时不输出（G7） |
