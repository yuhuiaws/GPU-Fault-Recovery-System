...
**控制面目前不对 `...-outcome-unknown` 做特殊分支**：它和普通失败一样进升级路径，
所以人工确认目前依赖这个 `status_source` 字符串本身，而不是自动分叉。
...
| `COMMAND_ID_REUSED` | 409 | ✗ | ✓ | 同一 `command_id` 换了 operation、`gpu_uuids` 或 `parameters` |
