...
内，也不需要为它们设计覆盖——`GPU_FAULT_COMPLETION_OBSERVE_UNMANAGED` 的只观察模式因此
不承担心跳（该模式下全集群 Pod 都被计数，心跳不会发出，集群保持 `UNKNOWN`），不再单独
支持；（2）心跳把 `IDLE` 交给无 checkpoint 的 REBOOT/RESET 路径，所以它宁可少发也不能多发。
