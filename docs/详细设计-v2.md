...
- **发布引擎的接入**：`bootstrap_gpu_target` 与 `join_cluster` 在 DCGM exporter 之后应用采集器；升级计划的
  ...
  规则命名空间的内容），`restore_observability_with_dataplane` 在 `observability_restore` 阶段先恢复控制面快照，
  再**按快照的键**逐集群 apply 快照对象、删除快照里不存在的对象、重启；候选版新加进配置、快照从未见过的集群缩到 0；
  最后恢复或删除规则命名空间。整份快照先校验再动任何对象；阶段返回的记录由阶段运行器持久化到
  `rollback_timing.phases.observability_restore.details.dataplane_adot`（`path: snapshot|previous-image`，
  `clusters: {<id>: restored|removed|scaled-to-zero|candidate-manifest-with-previous-image}`）。`previous-image`
