...
- 注册表在 `src/gpu_fault/schema_migrations.py`，每项是
  ...
  **12 个版本**；v7 是 `processor-retry-schedule-and-lane-policy`，v12 是
...
多进程采样只会漏报）与 `GpuFaultRegionalRegistrySecretDrift`（warning，漂移持续 15 分钟）。
...
的指标族一致。部署侧由 `gpu-fault-admin deploy --grafana {enabled,disabled,create}`（默认 enabled）
在 AMP 安装之后接管：区域取 CPU 集群 ARN，解析顺序为显式 `--grafana-workspace-id` → 站点 tag →
该区域唯一 ACTIVE workspace（首次采用时打 `gpu-fault:site-id` tag）→ 多个未标记时报错、没有时报错
（`create` 才新建，需要 Identity Center）；然后用短期服务账号 token 建 `gpu-fault-amp` 数据源
...
用例、45 个 Store codec kind、19 类通知）都是通过装配真实 app、实例化 Store codec 或读取
