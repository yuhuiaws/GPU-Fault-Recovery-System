...
- **渲染与跳过**：`regional_release_rendering.render_dataplane_adot_manifest` 只做占位符替换，
  ...
  `rollout status`）。发布引擎调用这一对的钩子随本批接入（F10）。
...
- 三种长时可变更的重装操作（`REMEDIATE_DRIVER`、`UPDATE_SOFTWARE_FIRMWARE`、
  ...
  仍派生带后缀的 id 且不回读，所以三种重装操作有 PENDING/WAITING 步骤时**发布工具拒绝**开事务：
  `upgrade_release`（新事务、resume、supersede 都从这里进）与 `rollback_release`（含自动回滚）在
  开事务前各读一次控制面 store（探针 `inflight_installs`，与远端命令空闲检查同一条
  `exec_cpu_ingress_probe` 路径），有在飞的重装步骤即拒绝并点名 workflow、操作、步骤与节点；
  store 读不到同样拒绝（fail closed）。只有 `gpu-fault-admin deploy --allow-inflight-installs`
  （环境变量 `GPU_FAULT_RELEASE_ALLOW_INFLIGHT_INSTALLS=1`）能越过——仍尽量读，读到什么就记
