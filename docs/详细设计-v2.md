...
- 三种长时可变更的重装操作（`REMEDIATE_DRIVER`、`UPDATE_SOFTWARE_FIRMWARE`、
  ...
  **已派发的 BLOCKED 行永不再执行**：执行器对 BLOCKED 直接返回既有结果，运维杠杆只会把它关到 SUPERSEDED 且拒绝带
  ...
  上 exec 在 kubectl 层失败——kubelet 挂、缺 `pods/exec` RBAC、镜像无 `sh`；或 `get pod` 本身失败）——活着的
  派发器问不到，所有模式拒绝、文案点名 kubectl 而非 store、同意变量可越过并记 `inflight-installs-unchecked` 加原因。
  判定 `{checked, verdict, reason, steps, inflight_count, scanned, checked_at}` 在升级与回滚两侧都于首个 checkpoint 前
  ...
  两种情形：点名列出的在飞重装（记 `inflight-installs-overridden steps=…`）与探针没能运行（记
  `inflight-installs-unchecked`）。自动回滚（`rollback_release(automatic=True)`，引擎内部以 `--automatic` 标记，
