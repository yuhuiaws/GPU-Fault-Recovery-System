...
- **发布引擎的接入**：`bootstrap_gpu_target` 与 `join_cluster` 在 DCGM exporter 之后应用采集器；升级计划的
  ...
  规则命名空间的内容），控制面一半仍在 `observability_restore` 阶段恢复；数据面一半是独立阶段，排在 `cpu_restore`
  之后（有自己的 checkpoint），**按快照的键**逐集群 apply 快照对象、删除快照里不存在的对象，apply 输出全为 `unchanged`
  ...
  `failed: …` 并继续其它集群与规则命名空间，阶段末统一抛错（回滚如实 FAILED，resume 重跑该阶段）；最后恢复或删除规则
  命名空间（存在性以 `ResourceNotFoundException` 判定，其它 describe 失败抛错；非 ACTIVE 状态先等待稳定；安装脚本删除后
  等到 ResourceNotFound）。整份快照——两个平面——先校验再动任何对象；阶段返回的记录由阶段运行器持久化
  ...
  `bootstrap`、`join_cluster`、`remove_cluster` 结束时调用 `_apply_observability` 重渲染逐集群规则（否则 bootstrap 出来
