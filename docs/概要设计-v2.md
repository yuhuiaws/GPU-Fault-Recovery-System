...
按闭环讲，每一环缺什么会直接暴露出来。
本文最近一次按源码、生成清单和测试契约逐项核对的日期为 **2026-09-15**。
...
flowchart LR
    ...
    I["③ 隔离<br/>cordon + NoSchedule taint<br/>阻止新负载调度"]
    M["④ 停止负载<br/>停止整个分布式 attempt"]
    R["⑤ 修复<br/>reset / reboot<br/>驱动 / 插件<br/>健康 warm-spare 切换"]
    V["⑥ 验证<br/>只认动作后的新样本"]
    B["⑦ 重新纳管<br/>解除隔离 + 恢复训练"]
...
③ 隔离、④ 停止负载、⑤ 修复三环，按动作目标区分职责：

- **③ 隔离**：`MARK_UNSCHEDULABLE` / `QUARANTINE` 设置 cordon 与自有
  `NoSchedule` 污点，阻止新负载调度，不负责停止已经运行的训练。
- **④ 停止负载**：`STOP_WORKLOADS` 停止受影响的整个分布式 attempt，
  为后续修复腾出执行条件；这一步不依赖是否有 warm spare，也不是在线迁移训练。
- **⑤ 修复**：执行 reset、reboot、驱动/插件修复，或通过 `REPLACE_NODE`
  切换到已纳管的健康 warm spare。热备切换恢复的是计算容量，不代表原故障节点
  已修复；原节点保持隔离，后续验证和重新纳管面向实际恢复目标。

代码依据为 `src/gpu_fault/orchestration/workflow_builder.py::WorkflowBuilder.catalog_operations`
和 `src/gpu_fault/adapters/kubernetes/adapter.py::KubernetesWorkflowAdapter.execute`。
对需要节点修复且仍有活跃训练的路径，先隔离，再停止负载，再执行修复；
没有活跃负载时不额外增加停止步骤。图展示主线职责，具体步骤仍由策略、证据和
Runtime Profile 决定，不能把七阶段理解为每个事件都必须完整执行的固定模板。

**闭环的两个入口。** 常态入口是 ①（信号先到）；另一个入口是训练进程先死、
再回查原因，见[被动训练终态总图](概要设计.md#passive-terminal-overview)。
Watcher 发现失败后异步触发“取证 + 停整个 attempt”，终态上报和决策不等待遏制完成；
需要新恢复 workflow 时，由前驱依赖安排先停后启。

被动入口从 ② 分流，**不要求每个终态都经过上图的节点修复主链**：
正常成功、用户停止或其他 incident 发起终止时不新建恢复；本系统被动遏制造成的
`STOPPED` 则继续判定。有可用 marker 时关联已有处置，已有 workflow 负责重启则不重复；
无 marker 但 allocation 非空且 Profile 允许重启时，不做硬件定因，直接规划预算内
单次重启。无 marker 路径缺少 allocation 或无可执行重启能力时，只取证并转 ⑦'，
不猜测节点，也不凭空生成节点隔离。细分条件见
[终态决策与恢复](详细设计.md#passive-terminal-decision)，各角色调用见
[被动终态时序](详细设计-v2.md#passive-terminal-sequence)。
...
| 级别 | 动作 | 状态 |
|---|---|---|
| 只告警 | AdvisoryNotification + 站点通知通道（`GPU_FAULT_NOTIFICATION_CHANNEL`：默认 `sns` 发布到站点 SNS topic，`ses` 邮件为 opt-in，`disabled` 只落 outbox） | 已实现，是所有级别的公共底座 |
...
| 人工步骤 | `CHECK_MECHANICALS`、`ESCALATE_SUPPORT` | 已实现；前者等待精确 annotation，后者生成合成 ticket ID、持久通知并经站点通知通道发出；没有外部工单系统 |
...
| 模型代码错误、超参问题 | 不做因果分析。训练终态 FAILED/TIMED_OUT 且无匹配硬件 marker 时**不做诊断**，直接在同一 job 的 restart budget 内重启一次（trigger `no-hardware-evidence:RESTART`）；profile 没有可执行的 workload restart 能力时只取证并升级人工（`no-hardware-evidence:ESCALATE`）；缺 allocation 时记 `allocation-missing:INCONCLUSIVE`，只升级不重启 |
...
| 采集器静默（健康不可确认） | 控制面读取已持久化的采集状态 | `collector-silence`，默认扫描间隔 60 s，非失联阈值 | 活跃集群中，ACTIVE 且 heartbeat lease 有效的 Agent 所需通道无成功记录，或距最近成功上报超过通道阈值（§2.5） | AdvisoryNotification + 站点通知通道；不因此创建恢复 workflow，**不将监控缺失视为健康** |
| 训练侧（终态与 hang） | Kubernetes watch、EFA 零流量 | `CompletionWatcher`、EFA 流量状态机 | 唯一 TerminalEvent；`HUNG_SUSPECTED`；空闲 pass（完整 list 后零个在跑 managed Pod、零个活跃 attempt）另发覆盖心跳 `POST /v1/attempts/coverage`（内置最多每 120 s 一条），让空闲集群判为 IDLE 而不是 UNKNOWN；忙集群由观测流本身证明 watcher 活着 | 终态按[被动流程总图](概要设计.md#passive-terminal-overview)分流：失败时异步遏制，决策不等待停训练完成；有可用 marker 时复用/编排处置，已有 workflow 负责重启则不重复恢复；无 marker 且 allocation 非空、Profile 可重启时才走预算内单次重启，否则取证并升级。hang 走**只读取证链** |

训练侧“可用 marker”不包含指向已存 incident 的纯诊断 marker；跳过后仍需检查
其他修复 marker。`after_incident` 重启在执行时等待原 incident `RECOVERED`，
并非匹配成功即可重启，完整规则统一见[详细决策](详细设计.md#passive-terminal-decision)。
...
（括号内为 rank 值；隔离、取证、验证、GPU 服务静默/恢复等 operation 不带 rank，
不参与排序），跨类型仲裁、workflow 合并、抢占和失败升级都用它比较。
planner 层的 `RecoveryAction` 在 `src/gpu_fault/models.py` 的
`RECOVERY_ACTION_RANK` 有一张同序的对应表（`RUN_DIAGNOSTICS` 10、
`MARK_UNSCHEDULABLE` / `STOP_WORKLOAD` 25、`DRAIN` / `QUARANTINE` 60、
`REPLACE_NODE` 70、`ESCALATE_OPERATOR` 80），用于被动终态路径挑选最强 marker。
...
这里的 Unknown 表达“监控缺失时不能确认健康”，不是统一的节点状态写入。
当前 `collector-silence` 任务只检查采集器静默并生成 `AdvisoryNotification`，
不会因此把节点写成 `UNKNOWN`，也不会创建恢复 workflow。

控制面默认以 **60 s 间隔**调度采集器静默检查，读取已持久化的成功上报时间，
不是向节点发送探测请求。60 s 可配置，是本轮结束到下轮调度的默认间隔，
不是失联判定阈值或告警送达时限；实际执行还取决于 Worker、Store 和周期任务租约。

检查范围限于活跃集群中 **ACTIVE 且 heartbeat lease 仍有效的 Agent** 的所需采集通道；
显式 disabled/masked 的采集服务不纳入。没有成功记录，或距最近成功上报超过该通道阈值时，
生成通知。默认阈值为 GPU 库存 180 s、GPU 指标/主机遥测 420 s、日志类 900 s，
通知按集群、节点和通道的默认 3600 s 时间桶去重。

Agent 心跳已过期的节点不参与这条静默扫描及对应指标统计；这不是健康结论，
应另看 Fleet readiness 和 stale-agent 指标。Executor、Watcher 等组件的失联也有各自
判据，不能把本任务描述为覆盖全部数据面组件的统一失联检测。实现边界见详细设计 v2 §2.3.1。
...
- **AMP/ADOT 只覆盖控制面自身指标**，且是可选增强。没有 AMP 的 region 设
  `GPU_FAULT_ENABLE_AMP=false`，安装脚本跳过 AMP/ADOT 资源（仅 legacy 脚本
  路径可达；`gpu-fault-admin` 一键部署恒写 `true`）。
- **业务通知不走 Prometheus**：workflow 结果和故障通知走控制面 Aurora outbox
  + 站点通知通道（默认 SNS topic，SES 为 opt-in）。collector 静默有两路：
  控制面采集器静默检查（默认间隔 60 s，范围与阈值见 §2.5）→ outbox 通知（不依赖 AMP），以及 AMP
  `GpuFaultCollectorSilent` 规则。ADOT→AMP→Alertmanager 与 outbox 两条路径的
  终点是**同一个站点 SNS topic**，但事实来源不同：前者是 `/metrics` 时序，
  后者是 Aurora 状态库。
...
| Notification outbox + Dispatcher | `src/gpu_fault/notification_service.py`、`src/gpu_fault/notifications/` | 通知投递必须与 workflow 结果解耦：SNS/SES 故障不能反向改变 step 结果 |
...
| Draining | **无独立状态**。先用 cordon + taint 隔离调度，再由 `STOP_WORKLOADS` 停止受管训练，不调 Eviction API（§7.4） | 建议的 Draining 在实现里对应 `ACTION_PENDING` 中的一组步骤，不是节点状态 |
...
| 多节点同一 attempt 的联动 | 聚合窗口内形成**一个** workflow；每个节点是一条 DAG 分支、各自执行本节点的 reset，单一 `join` 步在全部分支完成后只重启一次训练任务 |
...
| 预算侧 | 同一 `cluster_id/job_id` 共享**持久化** restart budget；耗尽时仍完成节点修复（cordon / stop / reset / validate 照常执行），只扣住 `RESTART_WORKLOAD`：它预先记 FAILED 并被 supersede，workflow 以 FAILED 收尾并发预算耗尽通知，作业需人工重新提交。文档明确写了"不要直接提高 restart budget 掩盖重复故障" |
| 通知侧 | 按小时桶去重（`GPU_FAULT_HOST_NOTIFICATION_COOLDOWN_SECONDS`）；瞬时 GPU warning 有独立 cooldown；**只有状态转换才生成 finding 和通知**，停在同一状态的持续异常不重复上报 |
...
flowchart TD
    ...
    G7 -- 是 --> G8{"RESTART_WORKLOAD 有<br/>job · attempt · 预算上下文？<br/>（源 GPU 数量缺失时先用<br/>attempt 观测值回填）"}
    G8 -- 否 --> X
    G8 -- 是 --> G9{"GPU 数量一致，或变化已由<br/>管理员精确 annotation 批准？"}
    G9 -- 否 --> P["等待管理员决策"]
    G9 -- 是 --> E["执行"]
```

G8 的"源 GPU 数量"不是硬门：`source_gpu_count` 缺失时先用该 attempt 的观测
GPU 数回填（`adapters/kubernetes/restart_operations.py` 的
`_observed_source_gpu_count`），回填不到才进入 G9，按"GPU 数量变化"处理——
发出通知并让 step 停在 `WAITING`，等管理员的精确 annotation。
...
| 多节点联动 | 同一 attempt 的多节点 SXID 在聚合窗口内合成一个 workflow：每节点一条 reset 分支，单一 join 重启 |
| MIG 爆炸半径 | **未实现**（§1.3） |
...
**节点隔离由 `MARK_UNSCHEDULABLE` / `QUARANTINE` 实现（cordon + 自有
`NoSchedule` taint）；停止负载由独立的 `STOP_WORKLOADS` 步骤执行。
全库不调用 Kubernetes Eviction API，没有 `kubectl drain`
...
- 集群 executor 的 RBAC 分两层。ClusterRole 只含集群级只读（`pods` /
  `jobs` / `pytorchjobs` / `jobsets` get/list/watch，spare 激活要跨命名空间
  证明节点空闲）加 `nodes` get/list/watch/**patch**（无 delete），见
  `deploy/dataplane/cluster-action-executor.yaml`。写动词——`pods`
  patch/**delete**、`jobs`/`pytorchjobs`/`jobsets` create/patch（无
  delete）——由 GPU rollout 按每个受管 workload 命名空间渲染
  Role + RoleBinding（`src/gpu_fault_release/regional_release_gpu_rollout.py`
  的 `EXECUTOR_WORKLOAD_RULES`），另在 device plugin 命名空间单独授予
  `pods delete`。因此 `GPU_FAULT_ALLOWED_WORKLOAD_NAMESPACES` 不再是唯一边界：
  越界命名空间的命令会被 executor 和 API server 同时拒绝。
...
> 对 Pod 不成立（executor 在受管 workload 与 device plugin 命名空间内有
> `pods delete`）。以本节为准，并见 §11 第 12 条。
...
`create_app()` 始终挂载 **92 条应用路由**；区域生产形态在这组路由上启用
"每条路由显式声明桶"的默认拒绝模型。五桶
`public` / `metrics` / `cluster-token` / `dual-credential` /
`execution-token` 当前分布 2 / 1 / 38 / 7 / 44（`public` 是 `/healthz` 与
`/livez`；数字以进程内 `create_app()` + `ExplicitAuthorizationRegistry.load()`
枚举为准）。新增路由未声明桶，
应用装配期即启动失败；运行期取不到桶一律 403——但**没有任何路由匹配的路径答 404**
（`ExplicitAuthorizationRegistry.route_exists`）：采集器 outbox 把 404 判为死信、把 403 当
token 轮换瞬态无限重放，NET-008 的 retired-channel 记录曾因此永不死信。
...
| 操作员队列 / 关闭已交人工的 incident | `GET /v1/incidents`、`GET /v1/incidents/{incident_id}`、`POST /v1/incidents/{incident_id}/close`（execution-token；只把 **ESCALATED** 关为 RECOVERED，仍有 workflow 打开时 409，重复关闭幂等） |
| 从管理员 shell 关闭 incident / 收尾 BLOCKED 记录 | `gpu-fault-admin workflow-reconcile --close-incident ID` / `--close-escalated` / `--close-quarantined`（后者只对节点已无 cordon、无本 incident 的 quarantine taint 与隔离 annotation 的 QUARANTINED incident 收尾，并剥离残留 annotation）；同一 verb 也关闭后续 workflow 已恢复的 BLOCKED 记录 |
| `CHECK_MECHANICALS` 处置 / 人工修好的节点回到状态库 | `gpu-fault-admin submit-remediation --incident-id ID --disposition {inspected,reset-gpu,reboot-node,quarantine,restore}`：确认检查、按记录编译新的 fenced 硬件处置 workflow，或（`restore`）为人工修好的 QUARANTINED 节点创建 validated restore workflow（`VALIDATE_GPU → VALIDATE_HOST → VALIDATE_FABRIC → RESTORE_SCHEDULING`，同一 incident 与 fencing token） |
| 声明 / 释放 warm spare | `gpu-fault-admin config spare --node NODE --declare` / `--release`（label + cordon，带 `--confirm` 确认串与 `--reference`） |
...
| 区域运维（领取/续租/回执/进度、spare 健康、证据与通知回写、集群注册表） | `/v1/regional/*` 共 20 个端点（`executors` 15、`registry` 4、`clusters` 1） |

**缺口：仍没有"隔离任意节点"与"终止 RUNNING workflow"的一等入口**。人工隔离当前靠
创建/推进 workflow 或直接在集群里 cordon+taint（后者绕过状态库，不推荐）；
`POST /v1/incidents/{id}/close` 与 `workflow-reconcile --close-*` 都拒绝仍有活
workflow 的 incident，正在跑的 workflow 只能等它 fail-closed 或被更强动作 `SUPERSEDED`。
已经补上的是三件事：ESCALATED incident 可经 `POST /v1/incidents/{id}/close` 关闭；
QUARANTINED 节点的人工修复经 `submit-remediation --disposition restore` 回到状态库；
终态 workflow 遗留的 `WAITING` 远端命令由 dispatcher 的周期 sweep
（`WorkflowDispatcher.sweep_stuck_records`）自动取消，不再需要人工命令。
...
| Kubernetes Event | **不写**。所有对外可见结论走 API + 站点通知通道，不写 Event 对象 |
...
- **站点通知通道是唯一的业务通知通道**：Aurora outbox + Notification Dispatcher +
  `GPU_FAULT_NOTIFICATION_CHANNEL`。`sns`（新站点默认）发布到站点 SNS topic；
  `ses`（opt-in）从已验证 sender 发邮件，已配置 SES sender 的旧站点升级时保留；
  `disabled` 只落 outbox。业务处理只创建幂等 notification；dispatcher 用
  owner/epoch/lease claim 并指数退避；旧 epoch 不能完成新 lease，
  **SNS/SES 故障不能反向改变 workflow step 结果**。
- **业务通知与 AMP Alertmanager 共用同一个 SNS topic**（`spec.health.snsTopicArn`），
  管理员只确认一次订阅。**没有 webhook、Slack、PagerDuty、Jira 集成**——要接它们，
  在该 topic 上加订阅，或在 outbox 上加 channel，不在业务路径上加调用。
- **"工单"目前是本地合成的标识**：`src/gpu_fault/adapters/support_escalation.py`
  生成 `vendor-ticket-<incident_id>` 并写进通知正文，**不调用任何工单系统 API**。
  通知里出现 "Ticket ID" 时，它是一个可供人工引用的关联串，不是外部工单号。
...
| Node Agent 防重放 | HMAC envelope + 命令 TTL + 目标节点校验 + agent generation + incident fencing token + 本地 SQLite ledger。ledger 以 `(command_id, attempt)` 为键做 exactly-once 重放：agent 重启时仍为 IN_PROGRESS 的行判为 interrupted、要求人工确认而不重跑；命令体（operation / GPU UUID / 参数摘要）不一致的重用 command_id 被 409 `COMMAND_ID_REUSED` 拒绝，该步终态、不重试 |
...
- 最小权限：控制面角色只拿站点所用通道的一项权限——`sns` 站点仅对站点 topic
  `sns:Publish`，`ses` 站点仅对已验证 sender `ses:SendEmail`；GPU 集群侧两者都不给。
...
| attempt observation / fleet deployment / diagnostic | 7 天 |
| node action ledger（节点侧 SQLite） | 30 天 |
| GPU finding 历史、热状态 latest/terminal | 30 天 |
| 控制记录（已关闭 incident 及其 workflow / plan / decision / event / notification / marker / remote command） | 30 天后归档到站点 S3 并删除，每 600 s 一批；`controlRecordRetentionDays: 0` 显式关闭 |
| GPU metrics batch / remote command | 24 h |
| processor lane | 1 h |
...
| 对象 | 手段 |
|---|---|
| collector 是否还在成功上报 | 两路：控制面默认以 60 s 间隔检查活跃集群中在线 Agent 的所需通道，按成功记录与通道阈值生成 AdvisoryNotification 走 outbox（范围与边界见 §2.5，不依赖 AMP）；AMP `GpuFaultCollectorSilent`（`gpu_fault_collector_silent_nodes > 0` 持续 5 min，critical）与 `GpuFaultDataplaneCollectorMissing` 规则。两路终点同为站点 SNS topic |
...
| 回滚 | schema 只向前（无 down migration），且**每个进程钉死精确版本**：`gpu_fault_schema_version` 必须等于进程自身的 `POSTGRES_SCHEMA_VERSION`，否则启动即 `RuntimeError`，旧进程读不了新 schema。带 schema 变更的 release 不能自动回滚（engine 在 `autoRollback: true` 下直接拒绝），必须用 `gpu-fault-admin deploy --accept-schema-change`：先 Aurora 快照，本事务 fail-forward，出问题靠快照恢复。`dual` 仅指 collector 热状态表 legacy/dedicated 双写（`hot_state_mode`），不是通用 schema 过渡机制 |
...
`tools/run_fault_test_cases.py`。当前 **333 条显式用例 + 2 个按 XID catalog
逐码生成的用例族**（`xid_catalog_rules` 生成器，覆盖 catalog 里每个 XID，
另有 B200 上下文变体，172 × 2 = 344 条），展开后 677 条，共 49 个 category
（47 个显式 + 2 个生成）。数字以加载 `testcases/fault-scenarios.yaml`
（`test_cases` + 展开 `generated_test_families`）为准；
`--list --include-manual --include-live` 只列 645 条，因为 32 条
`automation: command` 用例必须用 `--case` 显式选中才出现。

风险分级（下表只统计 333 条显式用例，决定能不能在真机跑）：

| risk | 条数 | 含义 |
|---|---:|---|
| `non-destructive` | 179 | 纯软件链路，可无条件执行 |
| `read-only-signal-replay` | 36 | 只重放信号，不动环境 |
| `live-non-destructive` | 36 | 真机执行但不改变状态 |
| `live-service-action` | 31 | 真机服务级动作 |
| `destructive` | 23 | 必须人工批准 |
| `live-workload-restart` | 7 | 真机重启训练 |
| `live-kernel-log-injection` | 6 | 真机写 kmsg |
| `destructive-provider-reboot` | 5 | 真机 provider reboot |
| `live-isolation` | 4 | 真机隔离 |
| `live-node-mutation` | 4 | 真机改节点 |
...
| 指标失联 / 杀 DCGM / Device Plugin / 驱动进程 | **部分覆盖**：`regional-collector` 21 条覆盖静默与失联判定；杀 DCGM/device plugin 进程的注入不在公开目录里 |
...
| 控制器修复中重启 | 已覆盖（`regional-high-availability` 8 条、`regional-preemption` 38 条含 lease 接管） |
| 重复与乱序事件 | 已覆盖（幂等、`attempt-generation-fence` 8 条、`regional-remote-command` 18 条含重复回执） |
| 修复失败人工接管 | 已覆盖（`destructive-acceptance`、`regional-destructive-acceptance` 各 7/26 条） |
...
| 10 | Drain 策略（关键 Pod / 系统 Pod / DaemonSet） | 不调 Eviction API；先用 cordon + taint 隔离调度，再单独停止受管训练任务 | 节点上的非受管 Pod 不受影响 | 这是刻意收窄的爆炸半径。若要真 drain，需要重新评估 RBAC 与 PDB 交互 |
| 11 | 工单系统集成 / Webhook | `vendor-ticket-<incident_id>` 是**本地合成串**，只写进通知正文；无 webhook | 需要人工把通知转成工单 | 若要集成，在站点 SNS topic 上加订阅或在 notification outbox 上加 channel，而不是在业务路径上加调用 |
| 12 | —（v1 自身的表述缺陷） | v1 §8.3 "RBAC 不授予 Node/Pod 删除权限"对 Node 成立、对 Pod 不成立（executor 在受管 workload 与 device plugin 命名空间内有 `pods delete`） | 评审时会误判权限面 | 按本文 §7.4 修正 v1 §8.3 的措辞 |
...
| 14 | "手工隔离/恢复/终止工作流" API | 有 marker、agent drain/revoke/reactivate、workflow execute、`POST /v1/incidents/{id}/close`（ESCALATED）、`workflow-reconcile --close-*`、`submit-remediation --disposition restore`（QUARANTINED 节点人工修复后回库）；**仍没有一等的"隔离任意节点"与"终止 RUNNING workflow"端点** | 人工隔离要么绕状态库，要么等 fail-closed | 补这两个显式端点，比让人 `kubectl taint` 更安全 |
