...
| 监控失联（Unknown） | 控制面反向判定 | 控制面 60 s 扫描 | ACTIVE 且 heartbeat lease 仍有效的通道超过静默阈值 | AdvisoryNotification + 站点通知通道；**不视为健康** |
...
但 lease 已过期的节点不参与指标或通知告警——否则已退役节点会形成永久静默误报。
同一节点、collector、channel 每小时最多一条通知。
...
- **业务通知不走 Prometheus**：workflow 结果和故障通知走控制面 Aurora outbox
  ...
  控制面 60 s 扫描 → outbox 通知（不依赖 AMP），以及 AMP
...
| 对象 | 手段 |
|---|---|
| collector 是否还在上报 | 两路：控制面 60 s 扫描 + 三档静默阈值 + AdvisoryNotification 走 outbox（§2.5，不依赖 AMP）；AMP `GpuFaultCollectorSilent`（`gpu_fault_collector_silent_nodes > 0` 持续 5 min，critical）与 `GpuFaultDataplaneCollectorMissing` 规则。两路终点同为站点 SNS topic |
