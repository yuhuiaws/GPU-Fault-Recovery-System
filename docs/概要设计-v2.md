...
| 训练侧（终态与 hang） | Kubernetes watch、EFA 零流量 | `CompletionWatcher`、EFA 流量状态机 | 唯一 TerminalEvent；`HUNG_SUSPECTED`；每次全量扫描后另发覆盖心跳 `/v1/workload-coverage`，让空闲集群判为 IDLE 而不是 UNKNOWN | 终态走 containment；hang 走**只读取证链**。匹配 marker 时诊断类 marker（取证/诊断/验证）不使其 incident 拥有作业恢复，terminal 走快速诊断 |
