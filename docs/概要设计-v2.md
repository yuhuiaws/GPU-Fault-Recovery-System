...
`src/gpu_fault/models.py` 的 `WorkflowOperation` 共 32 个取值；生产控制面
`GPU_FAULT_ALLOWED_OPERATIONS` 白名单收了 31 个（见
`deploy/control-plane/regional/generated/`），节点侧
应用裸默认的 `GPU_FAULT_NODE_ALLOWED_OPERATIONS` 只有 `VERIFY_NO_GPU_CLIENTS`；
节点安装脚本在启用 Agent 时写入 10 项基础 operation，再按 EFA、Field Diagnostic、
...
| 自动恢复成功率 | workflow 终态可统计，但没有定义为验收指标 |
| 重新纳管正确率 | 同上 |
...
| 训练侧（终态与 hang） | Kubernetes watch、EFA 零流量 | `CompletionWatcher`、EFA 流量状态机 | 唯一 TerminalEvent；`HUNG_SUSPECTED` | 终态走 containment；hang 走**只读取证链** |
...
建议列了九个模块。逐个映射到真实代码位置，并补上建议没提但实际存在的五个。
...
| 关联分析 | `src/gpu_fault/policy/`（XID companion 关联）、`src/gpu_fault/completion_controller.py`（attempt 关联） | 已实现 |
...
| Restart Guard | 原子预留重启预算 + 前后 GPU 数量校验 | 抗震荡的主力，见 §6.6 |
...
| Regional Cluster Registry + Remote Command | `src/gpu_fault/regional.py`、`src/gpu_fault/cluster_executor.py` | 跨信任边界下发协议，是"控制面不持有 kubeconfig"的落地方式 |
...
主要领域对象与幂等键见 v1 §6.1（10 个对象）。必须以 `(region, cluster_id, …)`
...
| 集群级并发上限 | executor 侧 `max_concurrent_commands` 默认 5、允许 1..25；HyperPod 批量 mutation `max_batch_size` 上限 25，超限报 "batch exceeds limit" |
...
| 门 | 实现 |
|---|---|
| 自动动作白名单 | 控制面 `GPU_FAULT_ALLOWED_OPERATIONS`（生产 31 项）+ 节点 `GPU_FAULT_NODE_ALLOWED_OPERATIONS`（应用裸默认 1 项；安装器写入 10 项基础列表并按能力追加）。两层都不在白名单 → 不执行 |
...
发布侧另有真正的 dry-run（`deploy/control-plane/regional/rollout_regional_release.py`
...
区域形态真实注册 **81 条路由**，按功能分组见 v1 §6.3；授权是"每条路由显式声明桶"
的默认拒绝模型，五桶 `public` / `metrics` / `cluster-token` / `dual-credential` /
`execution-token` 当前分布 1 / 1 / 36 / 7 / 36。新增路由未声明桶，
应用装配期即启动失败；运行期取不到桶一律 403。
...
| **缺口** | `/metrics` **没有通知类指标**（邮件积压、失败、退休数量都看不到），通知链路自身的可观测性是当前最薄的一环 |
...
| 维度 | 实现 |
|---|---|
| Agent / wheel 一致性 | `src/gpu_fault/fleet_compatibility.py` 的 `FleetCompatibilityPolicy` + `rollout_compatibility_reasons()` + `pin_value_is_accepted()`；protocol 版本与 artifact digest 双 pin |
...
`tools/run_fault_test_cases.py`。当前 **288 条显式用例 + 2 个按 XID catalog
逐码生成的用例族**（`xid_catalog_rules` 生成器，覆盖 catalog 里每个 XID，
另有 B200 上下文变体），共 48 个 category。

风险分级（决定能不能在真机跑）：

| risk | 条数 | 含义 |
|---|---:|---|
| `non-destructive` | 173 | 纯软件链路，可无条件执行 |
| `read-only-signal-replay` | 34 | 只重放信号，不动环境 |
| `live-non-destructive` | 29 | 真机执行但不改变状态 |
| `destructive` | 17 | 必须人工批准 |
| `live-service-action` | 15 | 真机服务级动作 |
| `live-kernel-log-injection` | 5 | 真机写 kmsg |
| `live-workload-restart` | 6 | 真机重启训练 |
...
| 控制器修复中重启 | 已覆盖（`regional-high-availability` 6 条、`regional-preemption` 31 条含 lease 接管） |
| 重复与乱序事件 | 已覆盖（幂等、`attempt-generation-fence` 8 条、`regional-remote-command` 16 条含重复回执） |
| 修复失败人工接管 | 已覆盖（`destructive-acceptance`、`regional-destructive-acceptance` 各 6/15 条） |
...
| 5 | 误隔离率、恢复成功率、重新纳管正确率、事件丢失率、端到端发现时延 | 均**未定义为可度量指标** | 无法证明闭环真的闭合，只能证明每段不超时 | 优先补这五个指标的分母定义与 `/metrics` 埋点 |
