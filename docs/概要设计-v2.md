...
| 训练侧（终态与 hang） | Kubernetes watch、EFA 零流量 | `CompletionWatcher`、EFA 流量状态机 | 唯一 TerminalEvent；`HUNG_SUSPECTED`；空闲 pass（完整 list 后零个在跑 managed Pod、零个活跃 attempt）另发覆盖心跳 `POST /v1/attempts/coverage`（内置最多每 120 s 一条），让空闲集群判为 IDLE 而不是 UNKNOWN；忙集群由观测流本身证明 watcher 活着 | FAILED/TIMED_OUT 终态走 containment + 预算内单次重启；hang 走**只读取证链**。匹配 marker 时，指向已存 incident 的诊断类 marker（只观察不修复：取证/诊断/验证）被跳过，terminal 按其余 marker 决定；匹配到修复类 marker 而其 incident workflow 不含 `RESTART_WORKLOAD` 时，planner 在该 incident 之后单独编排一次重启（`after_incident`，执行时等 incident RECOVERED）；没有任何可信 marker 匹配则走预算内单次重启 |
