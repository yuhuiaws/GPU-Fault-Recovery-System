...
**Files:** Modify `deploy/dataplane/hyperpod-dcgm-exporter.yaml`, `deploy/dataplane/node-installer-reconciler.yaml`, `deploy/dataplane/kubernetes-node-resource-collector.yaml`, `deploy/dataplane/optional/*.yaml`, `deploy/dataplane/gpu-metrics-collector.yaml` (delete, and fix the tests that assert it), `deploy/systemd/gpu-fault-dcgm-exporter.service`, `deploy/systemd/gpu-fault-node-agent.service`, `src/gpu_fault_release/regional_gpu_bootstrap.py`; Test `tests/test_deploy_layout.py`, `tests/test_installation_resources.py`, `tests/deploy/**`, `tests/regional/**` tests that render the exporter manifest.
...
- Delete `deploy/dataplane/gpu-metrics-collector.yaml` and update the tests that assert it (`test_in_cluster_collectors_mount_writable_state_directory` etc.).
