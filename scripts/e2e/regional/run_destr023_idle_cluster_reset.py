...
1. preflight refuses a cluster with any managed Pod or the coverage canary
   Job, and a control plane whose store has no ``get_workload_coverage``;
2. before the injection the runner waits until no attempt observation is
   younger than the freshness window while the heartbeat stays fresh and the
   node reads IDLE -- the coverage probe is recorded every 15 s;
...
# release before CP-8 has no ``get_workload_coverage``; that is reported, not
# raised, so the preflight can name the missing deploy.
COVERAGE_PROBE = r"""
import json
import sys
...
supported = hasattr(store, "get_workload_coverage")
heartbeat = store.get_workload_coverage(cluster_id) if supported else None
...
print(json.dumps({
    ...
    "heartbeat_age_seconds": (
        (now - heartbeat.scanned_at).total_seconds()
