...
def _ensure_private_zone(
    ...
    *,
    ...
    for cluster in gpu_clusters:
        association = (cluster.region, cluster.vpc_id)
        if association in associated_vpcs:
            associations.append(
                {
                    "vpc_id": cluster.vpc_id,
                    "vpc_region": cluster.region,
                    "ownership": "CREATED",
                }
            )
            continue
        result = subprocess.run(
            [
                ...
                zone_id,
                "--vpc",
                f"VPCRegion={cluster.region},VPCId={cluster.vpc_id}",
                "--comment",
...
def bootstrap_from_arns(
    ...
    *,
    ...
    _run_parallel(
        {
            "pod_identity_agent": lambda: _ensure_pod_identity_agent(
                active_runner,
                cpu,
                site_id,
            )
    ...
    for cluster in gpu_clusters:
        ...
        second_phase_tasks[f"node_keys:{cluster_id}"] = node_key_task
