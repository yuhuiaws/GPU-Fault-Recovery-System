...
class FleetRegistry:
    ...
    def _create_deployment(self, request: FleetDeploymentRequest) -> FleetDeployment:
        now = self.now()
        ...
        agents = {
            ...
            for record in self.store.list_agents(request.cluster_id)
        }
        deployment_nodes = []
        for node_id in nodes:
            ...
            ready = (
                record is not None
                and record.lifecycle_state is AgentLifecycleState.ACTIVE
        ...
        deployment = FleetDeployment(
            ...
            status=self._deployment_status(deployment_nodes),
            created_at=now,
            updated_at=now,
        )
