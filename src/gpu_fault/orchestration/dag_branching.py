...
class DagBrancher:
    ...
    def node_branch_covers_fault_scope(
        ...
        intent = self.step_indexes_merge_intent(workflow, indexes)
        if intent.intersection(self.arbiter.NODE_WIDE_RECOVERY_OPERATIONS):
            return True
        if not gpu_uuids:
            return True
        covered: set[str] = set()
    ...
    def dag_join_accepts_new_branch(self, workflow: WorkflowRequest) -> bool:
        restart_index = next(
            (
                index
                for index, step in enumerate(workflow.official_steps)
    ...
    def _terminal_superseded(
        ...
        for index, step in enumerate(steps):
            if step.operation is WorkflowOperation.RESTART_WORKLOAD:
                ...
                step.operation is WorkflowOperation.RESTORE_SCHEDULING
                and set(step.node_ids) & nodes
                and index not in existing.completed_step_indexes
                and not any(
    ...
    def _attach_candidate_branch(
        ...
        if stop_is_open:
            ...
            suffix = [step for _, step in indexed if id(step) not in prefix_ids]
        else:
            prefix = []
        ...
        return suffix_tail or prefix_tail
    ...
    def widen_parallel_job_branch(
        ...
        for index, step in enumerate(existing.official_steps):
            ...
            if isinstance(raw, dict):
                ...
                mapping[node_id] = sorted(set(mapping.get(node_id, [])) | gpu_uuids)
