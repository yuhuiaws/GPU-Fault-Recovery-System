...
from gpu_fault.admin_aws_cleanup import ResourceCleaner, write_json_atomic
...
def _detach_network(
    ...
    *,
    ...
    revoked = []
    for eip in sorted(set(target_network["nat_eips"]) - remaining_eips):
        changed = _idempotent_aws(
            ...
            description=f"cannot revoke NLB ingress for {eip}",
        )
        if changed:
            revoked.append(eip)
    detached_vpc = None
    if (
        ...
        changed = _idempotent_aws(
            [
                ...
                "--hosted-zone-id",
                str(config["dns"]["hosted_zone_id"]),
                "--vpc",
                f"VPCRegion={region},VPCId={target_network['vpc_id']}",
            ],
            not_found=("VPCAssociationNotFound",),
            description="cannot detach GPU VPC from private hosted zone",
        )
        if changed:
    ...
    return {"revoked_nat_eips": revoked, "detached_vpc_id": detached_vpc}


def _target_resource(resource: InstallationResource, cluster_id: str) -> bool:
    return (
        resource.resource_key.startswith(f"aws/iam/executor/{cluster_id}/")
        or resource.resource_key.startswith(f"cluster/{cluster_id}/")
        or resource.resource_key == f"aws/route53/vpc-association/{cluster_id}"
    )
...
def _remove_target_aws_resources(
    ...
    for resource in selected:
        if resource.delete_policy is InstallationResourceDeletePolicy.DELETE:
            ...
            detached.append(resource.resource_key)
...
def remove_cluster(
    ...
    *,
    ...
    if not _done(state, "DISCOVERED"):
        ...
        target_network = _cluster_network(
            active_runner,
            region=region,
        ...
        cpu_network = _cluster_network(
            active_runner,
            region=region,
            eks_arn=str(request.site.release_config["cpu_eks_arn"]),
        )
        evidence = {
            "target": target,
    ...
    if not _done(state, "KUBERNETES_REMOVED"):
        ...
        _delete_target_namespace(request.site, target)
        _complete(
            state_path,
            state,
            "KUBERNETES_REMOVED",
    ...
    if not _done(state, "CONTROL_REGISTRY_REMOVED"):
        ...
        _complete(
            state_path,
            state,
            "CONTROL_REGISTRY_REMOVED",
            {"node_action_keys_removed": removed_keys},
        )
    ...
    if not _done(state, "VERIFIED"):
        _verify_target_absent(request, target)
        _verify_remaining_site(updated_site)
        _complete(
            state_path,
            state,
            "VERIFIED",
