...
if [[ "${ENABLE_NODE_AGENT}" == "true" ]]; then
    NODE_ALLOWED_OPERATIONS="COLLECT_HUNG_TRIAGE,COLLECT_DIAGNOSTIC_BUNDLE,RUN_DCGM_DIAGNOSTIC,QUIESCE_GPU_SERVICES,VERIFY_NO_GPU_CLIENTS,TRIGGER_HEALTH_SNAPSHOT,RESET_GPU,RESET_ALL_GPUS_NVSWITCHES,RESTORE_GPU_SERVICES,RESTART_FABRIC_MANAGER,COLLECTOR_OUTBOX_MAINTENANCE"
    if [[ "${ALLOW_FIELD_DIAGNOSTIC}" == "true" ]]; then
